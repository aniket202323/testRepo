...
-- PROCEDURE: [PR_Personnel].[usp_VerifyPersonMigration]

-- Stored procedure to perform a simple check that the migration of Person data 
-- succeeded.

CREATE PROCEDURE [PR_Personnel].usp_VerifyPersonMigration
@debug BIT = 1,                    -- if 1, print out all statements before executing
@test  BIT = 0                     -- if 1, do not make any changes

AS
BEGIN
	DECLARE @routineName VARCHAR(100)		= '[PR_Personnel].[usp_VerifyPersonMigration]'
	DECLARE @targetTableName VARCHAR(100)	= '[PR_Authorization].Person'
	DECLARE @sourceTableName VARCHAR(100)	= '[dbo].PersonLegacy'

	--Get the number of original records
	DECLARE @originalRecordCount INT
	SELECT @originalRecordCount = COUNT (*) FROM [dbo].PersonLegacy

	--Get the number of migrated records
	DECLARE @recordsAdded INT
	SELECT @recordsAdded = COUNT (*) FROM [PR_Authorization].Person
