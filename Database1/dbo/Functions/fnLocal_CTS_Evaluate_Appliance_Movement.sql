...
/*=====================================================================================================================
Local Function: fnLocal_CTS_Evaluate_Appliance_Movement
=====================================================================================================================
 ...
 Description			: Evaluation appliance movement to location
						  This function evaluates container movement to a location
...
-
 ...
 1.1		2022-01-28		F. Bergeron				Prevent user not in roles other 'Plant Apps Admin','Operator','Manufacturing Leader','Super User' to move appliances
 1.2		2022-02-07		F. Bergeron				New parameter on fnLocal_CTS_Location_Status
...
SELECT * FROM fnLocal_CTS_Evaluate_Appliance_Movement(1268670,10420,NULL,1596)

====================================================================================================================== */

CREATE   FUNCTION [dbo].[fnLocal_CTS_Evaluate_Appliance_Movement]
(
@ApplianceId			INTEGER,
...
BEGIN
	...
	DECLARE 	@FAppliance_locations TABLE
	(
		PU_Id						INTEGER,
		PU_Desc						VARCHAR(50),
		LType						VARCHAR(25)
...
=====================================================================================================================*/	
	...
	LEFT JOIN dbo.products_base PB							WITH(NOLOCK)	ON PB.prod_id = Last_product_id;

	UPDATE	@ApplianceStatus 
	SET Hardware_Status = PS.prodstatus_desc
...
=====================================================================================================================*/	
	...
	SELECT		Location_id,
				...
				Mover_User_AD,
				Err_Warn
	FROM		[dbo].[fnLocal_CTS_Appliance_Transitions](@ApplianceId, 0, NULL, NULL,'BACKWARD');
	...
	LEFT JOIN dbo.event_details ED				WITH(NOLOCK)	ON ED.event_id = EC.event_Id
	LEFT JOIN dbo.production_plan PP			WITH(NOLOCK)	ON PP.pp_id = ED.PP_id
	LEFT JOIN dbo.production_plan_starts PPST	WITH(NOLOCK)	ON PPST.pp_id = PP.pp_id
	LEFT JOIN dbo.Production_Plan_Statuses PPS	WITH(NOLOCK) 	ON PP.pp_status_id = PPS.pp_status_id
	LEFT JOIN dbo.Prod_Units_Base PUB			WITH(NOLOCK)	ON PUB.pu_id = E.PU_Id
	LEFT JOIN dbo.products_base PB				WITH(NOLOCK)	ON PB.prod_id = E.Applied_Product
	JOIN dbo.users_base UB						WITH(NOLOCK)	ON UB.user_id = E.user_id
...
=====================================================================================================================*/
	...
	BEGIN
		...
		(
			...
			SELECT -1, 	'Process order id is not found';
		GOTO The_End;
	END;
	...
	BEGIN
		...
		*/
		...
		BEGIN
			...
			SELECT  -1,  'Location is Dirty, movement is refused';
			GOTO The_End;
		END;
	END;
...
=====================================================================================================================*/
	...
	BEGIN
		...
		*/		
		...
		(		SELECT	CCM.Code
				FROM	dbo.Local_CTS_Product_Transition_Cleaning_Methods CPTCM WITH(NOLOCK)
				JOIN dbo.Local_CTS_Cleaning_Methods CCM 	ON CCM.CCM_id = CPTCM.CCM_id
				WHERE	From_Product_id =	(	SELECT Last_product_id 
												FROM @ApplianceStatus
											)
					AND To_product_id =		(
												SELECT Last_product_id 
		...
		BEGIN
			IF
				...
				(	SELECT		BOMFI.Prod_Id 
					...
					JOIN dbo.Bill_Of_Material_Formulation_Item BOMFI	WITH(NOLOCK) 	ON BOMFI.BOM_Formulation_Id = BOMF.BOM_Formulation_Id
					WHERE		PP.PP_Id = @DestProcessOrderId
				)
				OR
				((	SELECT	Last_product_id FROM	@ApplianceStatus ) 	=(	SELECT	Last_product_id FROM	@DestinationStatus	))
			BEGIN
...
=====================================================================================================================*/
	...
	BEGIN
		...
		/* REFUSE MOVEMENT */
			...
			SELECT  0,  'In use Appliance product, not compatible with In use location product ' ;
			GOTO The_End;
		END;
	END;
/*=====================================================================================================================
...
=====================================================================================================================*/
	...
	BEGIN
		/* LOOK IN MATRIX	*/
		SET @NextCleaning = (	SELECT	CCM.Code
								FROM	dbo.Local_CTS_Product_Transition_Cleaning_Methods CPTCM WITH(NOLOCK)
								JOIN dbo.Local_CTS_Cleaning_Methods CCM		ON CCM.CCM_id = CPTCM.CCM_id
		WHERE	From_Product_id =	(	SELECT Last_product_id 
										FROM @ApplianceStatus
									)
				AND To_product_id =	(	SELECT Last_product_id 
										FROM @DestinationStatus
												)
		);
		/* LOOK IN BOM */
		...
		SELECT  -1,  'Appliance is Dirty, movement is refused.  Minimum appliance cleaning is ' + @NextCleaning;
		GOTO The_End;
	END;
		...
		BEGIN
			IF 
				...
				INSERT INTO @Output 
					...
					SELECT  -1,  'No compatible product in BOM';
					GOTO The_End;
				END;
			END;
			ELSE
