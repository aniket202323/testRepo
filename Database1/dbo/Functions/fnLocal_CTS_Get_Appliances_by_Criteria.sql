...
--------------------------------------------------------------------------------------------------
-- Stored Procedure: fnLocal_CTS_Get_Appliances_by_Criteria
--------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------
-- ===========================================================================================
-- 1.0		2022-04-20		F. Bergeron				Initial Release 
--================================================================================================
--
--------------------------------------------------------------------------------------------------
-- TEST CODE:
...
SELECt * FROM  fnLocal_CTS_Get_Appliances_by_Criteria (NULL,NULL,NULL, NULL, NULL,NULL,'Bergeron.fe')


*/

CREATE   FUNCTION [dbo].[fnLocal_CTS_Get_Appliances_by_Criteria](
	...
	@CleaningInPlaceMessage			VARCHAR(500),
	@MovementMessage				VARCHAR(500),
	...
	IF (SELECT COUNT(1) FROM @F_Products) > 0 
		DELETE @Output WHERE  COALESCE(Appliance_product_Id,0) NOT IN(SELECT Product_id FROM @F_Products) 
	...
	WHERE	PS.prodStatus_desc IN ('CTS_Cleaning_Started','CTS_Cleaning_Completed')
			AND UDE.pu_id = Appliance_location_Id	
