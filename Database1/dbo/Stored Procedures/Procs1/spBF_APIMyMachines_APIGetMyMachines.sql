--Stored proc 
CREATE PROCEDURE [dbo].[spBF_APIMyMachines_APIGetMyMachines] 
 	 @UserId Int
AS 
...
IF EXISTS(Select 1 from User_Mymachines WHERE User_Id = @UserId AND Flag=1)
Begin
 	 Select Dept_Id,Dept_Desc,PL_Id,PL_Desc,PU_Id, PU_Desc,ET_Id, ET_Desc,Is_Slave from User_Mymachines WHERE User_Id = @UserId AND Flag=1
 	 Return
End
...
JOIN Departments de on de.Dept_Id = pl.Dept_Id
