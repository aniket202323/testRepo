...
@RSTypeList              - list of resultsets that should be returned
                            1 - Top N Downtime by Unit
                            2 - Top N Downtime by Location (Source Unit)
                            3 - Top N Downtime by Fault
                            4 - Top N Downtime by Reason (Level 1)
                            5 - Top N Downtime by Shift
                            6 - Top N Downtime by Crew
                            7 - Top N Downtime by Operator
                            8 - Top N Downtime by Product
                            9 - Unassigned Downtime by unit/operator
                           10 - Top N Downtime by Category
@N                       - Number of items to return for Top N result sets
@InTimeZone              - Ex: 'India Standard Time','Central Stardard Time'
*/
CREATE Procedure [dbo].[spBF_DowntimeGetReportData]
...
DECLARE   @DownTime TABLE (
 	 ...
 	 insert into @DownTime (Detail_Id, Start_Time, End_Time, Duration, Uptime, SourcePU, MasterUnit, Cause_Comment_Id,
 	  	  	  	  	  	    R1_Id, R2_Id, R3_Id, R4_Id, A1_Id, A2_Id, A3_Id, A4_Id, Fault_Id, Status_Id,Crew_Desc,Shift_Desc,Prodid,ppId,IsNPT,NPTDetId,NPTCatId)
 	 exec [dbo].[spBF_DowntimeGetAll] @StartTime, @EndTime, @UnitList, @InTimeZone, ',', 0, 1
 	 -- Get rid oz zero uptimes, it screws up the MTBF calculation
 	 Update @DownTime set Uptime = null where Uptime = 0
 	 ...
 	 FROM @DownTime d
 	 Join [dbo].[Timed_Event_Details] ted on ted.TEDet_Id = d.Detail_Id
 	 Left Join [dbo].[Prod_Units] u on u.PU_Id = d.MasterUnit
 	 Left Join [dbo].[Prod_Units] l on l.PU_Id = d.SourcePU
    Left Join [dbo].[Timed_Event_Fault] tef on tef.TEFault_id = d.Fault_id
    ...
    Left Join [dbo].[Event_Reasons] r4 on r4.event_reason_id = d.R4_Id
 	 ...
 	 Left Join [dbo].[Event_Reason_Category_Data] ercd on ercd.Event_Reason_Tree_Data_Id = ted.Event_Reason_Tree_Data_Id
    ...
    Left Join [dbo].[Event_Reason_Tree_Data] ertd on ertd.Event_Reason_Tree_Data_Id = ted.Event_Reason_Tree_Data_Id  
 	 ...
 	 Else
 	  	 Begin
 	  	  	 UPDATE @DownTime SET Operator = 'Unknown' Where Operator is null
 	  	 End
-------------------------------------------------------------------------------------------------
-- Total Downtime
-------------------------------------------------------------------------------------------------
...
Select @TotalDownTime = @TotalDownTime + coalesce((SELECT sum(Duration) From @DownTime),0)
if @TotalDownTime <= 1 set @TotalDownTime = 1
-------------------------------------------------------------------------------------------------
...
Declare @Summary Table (ID1 bigint null, ID2 bigint null,
 	  	  	  	  	  	 ...
 	  	  	  	  	  	 MTTR1 float NULL, MTTR2 float NULL,
 	  	  	  	  	  	 MTBF1 float NULL, MTBF2 float NULL,
 	  	  	  	  	  	 PercentTotal1 float NULL, PercentTotal2 float NULL,
 	  	  	  	  	  	 NumberOfEvents1 int NULL, NumberOfEvents2 int NULL)
-------------------------------------------------------------------------------------------------
-- Generate Result Set #1 - Top N Downtime by Unit
-------------------------------------------------------------------------------------------------
if Exists(select * from @RSTypes where TypeId = 1)
Begin
 	 Delete from @Summary
 	 insert into @Summary (ID1, Group1, Total1, MTTR1, MTBF1, PercentTotal1, NumberOfEvents1)
 	 Select MasterUnit, Unit, sum(Duration), avg(Duration), avg(Uptime), sum(Duration) / @TotalDowntime, count(distinct(Detail_Id))
 	   from @DownTime
 	 ...
 	 Select Top(@N) ID1                                          as 'UnitId',
 	  	  	  	    ...
 	  	  	  	    NumberOfEvents1                              as 'Events',
                    	  	    '' 	  	  	  	  	  	 as 'StartTime',
                    	  	    '' 	  	  	  	  	  	 as 'EndTime'
 	   from @Summary as DowntimeByUnit
 	   order by Total1 Desc
End
-------------------------------------------------------------------------------------------------
-- Generate Result Set #2 - Top N Downtime by Location (Source Unit)
-------------------------------------------------------------------------------------------------
if Exists(select * from @RSTypes where TypeId = 2)
Begin
 	 Delete from @Summary
 	 insert into @Summary (ID1, Group1, Total1, MTTR1, MTBF1, PercentTotal1, NumberOfEvents1)
 	 Select LocationId, Location, sum(Duration), avg(Duration), avg(Uptime), sum(Duration) / @TotalDowntime, count(distinct(Detail_Id))
 	   from @DownTime
 	 ...
 	 Select Top(@N) ID1                                          as 'LocationId',
 	  	  	  	    ...
 	  	  	  	    NumberOfEvents1                              as 'Events',
                    	  	    '' 	  	  	  	  	  	 as 'StartTime',
                    	  	    '' 	  	  	  	  	  	 as 'EndTime'
 	   from @Summary as DowntimeByLocation
 	   order by Total1 Desc
End
-------------------------------------------------------------------------------------------------
-- Generate Result Set #3 - Top N Downtime by Fault
-------------------------------------------------------------------------------------------------
if Exists(select * from @RSTypes where TypeId = 3)
Begin
 	 Delete from @Summary
 	 insert into @Summary (ID1, Group1, Total1, MTTR1, MTBF1, PercentTotal1, NumberOfEvents1)
 	 Select Fault_id, Fault, sum(Duration), avg(Duration), avg(Uptime), sum(Duration) / @TotalDowntime, count(distinct(Detail_Id))
 	   from @DownTime
 	 ...
 	 Select Top(@N) ID1                                          as 'FaultId',
 	  	  	  	    ...
 	  	  	  	    NumberOfEvents1                              as 'Events',
                    	  	    '' 	  	  	  	  	  	 as 'StartTime',
                    	  	    '' 	  	  	  	  	  	 as 'EndTime'
 	   from @Summary as DowntimeByFault
 	   order by Total1 Desc
End
-------------------------------------------------------------------------------------------------
-- Generate Result Set #4 - Top N Downtime by Reason (Level 1)
-------------------------------------------------------------------------------------------------
if Exists(select * from @RSTypes where TypeId = 4)
Begin
 	 Delete from @Summary
 	 insert into @Summary (ID1, Group1, Total1, MTTR1, MTBF1, PercentTotal1, NumberOfEvents1)
 	 Select R1_Id, Reason1, sum(Duration), avg(Duration), avg(Uptime), sum(Duration) / @TotalDowntime, count(distinct(Detail_Id))
 	   from @DownTime
 	 ...
 	 Select Top(@N) ID1                                          as 'ReasonId',
 	  	  	  	    ...
 	  	  	  	    NumberOfEvents1                              as 'Events',
                    	  	    '' 	  	  	  	  	  	 as 'StartTime',
                    	  	    '' 	  	  	  	  	  	 as 'EndTime'
 	   from @Summary as DowntimeByReason
 	   order by Total1 Desc
End
-------------------------------------------------------------------------------------------------
-- Generate Result Set #5 - Top N Downtime by Shift
-------------------------------------------------------------------------------------------------
if Exists(select * from @RSTypes where TypeId = 5)
Begin
 	 Delete from @Summary
 	 insert into @Summary (Group1, Total1, MTTR1, MTBF1, PercentTotal1, NumberOfEvents1)
 	 Select Shift_Desc, sum(Duration), avg(Duration), avg(Uptime), sum(Duration) / @TotalDowntime, count(distinct(Detail_Id))
 	   from @DownTime
 	   group by Shift_Desc
 	 ...
 	 Select Top(@N) Group1                                       as 'Shift',
 	  	  	  	    ...
 	  	  	  	    NumberOfEvents1                              as 'Events',
                    	  	    '' 	  	  	  	  	  	 as 'StartTime',
                    	  	    '' 	  	  	  	  	  	 as 'EndTime'
 	   from @Summary as DowntimeByShift
 	   order by Total1 Desc
End
-------------------------------------------------------------------------------------------------
-- Generate Result Set #6 - Top N Downtime by Crew
-------------------------------------------------------------------------------------------------
if Exists(select * from @RSTypes where TypeId = 6)
Begin
 	 Delete from @Summary
 	 insert into @Summary (Group1, Total1, MTTR1, MTBF1, PercentTotal1, NumberOfEvents1)
 	 Select Crew_Desc, sum(Duration), avg(Duration), avg(Uptime), sum(Duration) / @TotalDowntime, count(distinct(Detail_Id))
 	   from @DownTime
 	   group by Crew_Desc
 	 ...
 	 Select Top(@N) Group1                                       as 'Crew',
 	  	  	  	    ...
 	  	  	  	    NumberOfEvents1                              as 'Events',
                    	  	    '' 	  	  	  	  	  	 as 'StartTime',
                    	  	    '' 	  	  	  	  	  	 as 'EndTime'
 	   from @Summary as DowntimeByCrew
 	   order by Total1 Desc
End
-------------------------------------------------------------------------------------------------
-- Generate Result Set #7 - Top N Downtime by Operator
-------------------------------------------------------------------------------------------------
if Exists(select * from @RSTypes where TypeId = 7)
Begin
 	 Delete from @Summary
 	 insert into @Summary (Group1, Total1, MTTR1, MTBF1, PercentTotal1, NumberOfEvents1)
 	 Select Operator, sum(Duration), avg(Duration), avg(Uptime), sum(Duration) / @TotalDowntime, count(distinct(Detail_Id))
 	   from @DownTime
 	   group by Operator
 	 ...
 	 Select Top(@N) Group1                                       as 'Operator',
 	  	  	  	    ...
 	  	  	  	    NumberOfEvents1                              as 'Events',
                    	  	    '' 	  	  	  	  	  	 as 'StartTime',
                    	  	    '' 	  	  	  	  	  	 as 'EndTime'
 	   from @Summary as DowntimeByOperator
 	   order by Total1 Desc
End
-------------------------------------------------------------------------------------------------
-- Generate Result Set #8 - Top N Downtime by Product
-------------------------------------------------------------------------------------------------
if Exists(select * from @RSTypes where TypeId = 8)
Begin
 	 Delete from @Summary
 	 insert into @Summary (ID1, Group1, Total1, MTTR1, MTBF1, PercentTotal1, NumberOfEvents1)
 	 Select ProdId, Product, sum(Duration), avg(Duration), avg(Uptime), sum(Duration) / @TotalDowntime, count(distinct(Detail_Id))
 	   from @DownTime
 	 ...
 	 Select Top(@N) ID1                                          as 'ProductId',
 	  	  	  	    ...
 	  	  	  	    NumberOfEvents1                              as 'Events',
                    	  	    '' 	  	  	  	  	  	 as 'StartTime',
                    	  	    '' 	  	  	  	  	  	 as 'EndTime'
 	   from @Summary as DowntimeByProduct
 	   order by Total1 Desc
End
-------------------------------------------------------------------------------------------------
-- Generate Result Set #9 - Unassigned Downtime by unit/operator
-------------------------------------------------------------------------------------------------
...
Begin
 	 ...
 	 update @Summary set NumberOfEvents1 = (select COUNT(distinct(Detail_Id)) from @DownTime where Unit = Group1 and Reasons_Completed = 0)
...
End
-------------------------------------------------------------------------------------------------
-- Generate Result Set #10 - Top N Downtime by Category
-------------------------------------------------------------------------------------------------
if Exists(select * from @RSTypes where TypeId = 10)
Begin
 	 Delete from @Summary
 	 insert into @Summary (ID1, Group1, Total1, MTTR1, MTBF1, PercentTotal1, NumberOfEvents1)
 	 Select CategoryId, Category, sum(Duration), avg(Duration), avg(Uptime), sum(Duration) / @TotalDowntime, count(distinct(Detail_Id))
 	   from @DownTime
 	 ...
 	 Select Top(@N) ID1                                          as 'CategoryId',
                   ...
                   '' 	  	  	  	  	  	  	  	 as 'StartTime',
                   '' 	  	  	  	  	  	  	  	 as 'EndTime'
 	   from @Summary as DowntimeByCategory
 	   order by Total1 Desc
End
-------------------------------------------------------------------------------------------------
...
Begin
 	 select * from @DownTime Order by MasterUnit,Start_Time
End
