...
7 - List of all Associated tag of a unit                                                                             EXECUTE [spBF_GetPlantModelByAsset] null, 7
8 - List of all Associated tag of a group                                                                            EXECUTE [spBF_GetPlantModelByAsset] null, 8
*/
CREATE PROCEDURE [dbo].[spBF_ProcAnz_GetPlantModelByAsset] 
       @AssetId             INT = NULL,
       @AssetType           INT = NULL,
       @pageNum             INT = 1,
       @pageSize            INT = 20
...
BEGIN
       ...
       BEGIN
            IF EXISTS (
            SELECT 1 
            FROM dbo.Departments_Base WITH(NOLOCK) 
            WHERE Dept_Id > 0)
            BEGIN
       ...
       BEGIN
            ...
            WHERE Dept_Id = @AssetId AND PL_Id > 0) -- List of all lines of a particular department
            BEGIN
                INSERT INTO @Assets(AssetType, AssetId, AssetName, IsOpenable, HasChildren)
                SELECT AssetType = @AssetType,
                    AssetId = Line.PL_Id,
                    AssetName = Line.PL_Desc,
                    IsOpenable = 1,
                    HasChildren = CASE WHEN (
                    SELECT COUNT(*) 
                    FROM dbo.Prod_Units_Base WITH(NOLOCK) 
                    WHERE PL_Id = Line.PL_Id) > 0 THEN 1 ELSE 0 END
                FROM  dbo.Prod_Lines_Base Line WITH(NOLOCK)     
                WHERE Line.PL_Id > 0
                AND Dept_Id = @AssetId
                SELECT * FROM @Assets 
       ...
       BEGIN
            IF EXISTS (
            SELECT 1 
            FROM dbo.PU_Groups WITH(NOLOCK) 
            WHERE PU_Id = @AssetId 
            AND PUG_Id > 0)
            BEGIN
                INSERT INTO @Assets(AssetType, AssetId, AssetName, IsOpenable, HasChildren)
    ...
    END
END
GRANT  EXECUTE  ON [dbo].[spBF_ProcAnz_GetPlantModelByAsset]  TO [ComXClient]
