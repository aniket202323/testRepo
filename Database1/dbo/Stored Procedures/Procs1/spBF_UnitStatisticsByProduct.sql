...
*/
CREATE Procedure [dbo].[spBF_UnitStatisticsByProduct]
@Unit                    int,
...
DECLARE @PagedProducts TABLE  ( RowID int IDENTITY, ProdDesc nvarchar(100),PercentOfTotal Float)
...
select 	 DISTINCT   ProductId,
 	  	 ...
 	  	 AvaliableRate,
 	  	 OEE
  from 	 fnBF_wrQuickOEEProductSummary(@Unit,@StartTime,@EndTime,@InTimeZone,@FilterNonProductiveTime,4,0)
END
ELSE
 	 BEGIN
...
INSERT INTO @PagedProducts (ProdDesc,PercentOfTotal)
 	 SELECT ProdDesc,PercentOfTotal
...
BEGIN
 	 Select 	 a.ProductId,Product = b.Prod_Desc, 	 a.IdealSpeed,a.ActualSpeed, 	 a.IdealProduction, 	 
 	  	 ...
 	  	 JOIN @PagedProducts  c on b.Prod_Desc = c.ProdDesc  and c.PercentOfTotal = a.PercentOfTotal
