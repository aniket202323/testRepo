CREATE Procedure dbo.spCMN_GetUnitStatistics
@Unit int,
@StartTime datetime, 
@EndTime datetime,
...
If @ProductionSpecification Is Not Null
  Begin
    Select  @ProductionProperty = prop_id 
 	 ...
 	 Begin
 	  	 Select * into #UnitTimes from @UnitTimes
 	  	 Delete From @UnitTimes
 	  	 --Insert Into @UnitTimes(StartTime, EndTime, ProductId)
 	  	 -- 	 Select StartTime, EndTime, ProductId From @SpecChangeTable order by StartTime
 	  	 ;WITH S AS 
 	  	 (
 	  	  	  Select Distinct StartTime,ProductId from #UnitTimes
 	  	  	  Union
 	  	  	  Select Distinct EndTime,ProductId from #UnitTimes
 	  	  	  union 
 	  	  	  Select distinct StartTime,ProductId from @SpecChangeTable
 	  	  	  union
 	  	  	  Select distinct EndTime,ProductId from @SpecChangeTable
 	  	  	  Union 
 	  	 ...
 	  	 Insert Into @UnitTimes(StartTime, EndTime, ProductId)
 	  	 Select StartTime,LEAD(Starttime) Over (Order By Starttime) [EndTime],ProductId from S Order by Starttime
 	  	 UPDATE @UnitTimes SET StartTime =@StartTime WHERE StartTime < @StartTime
 	  	 UPDATE @UnitTimes SET Endtime =@EndTime WHERE EndTime > @EndTime
 	  	 DELETE FRom @UnitTimes WHERE StartTime = EndTime
 	  	 Delete from   @UnitTimes where EndTime IS NULL
 	  	 Delete from @UnitTimes where Endtime <@StartTime OR StartTime > @EndTime
 	 End
...
Select @TotalQualityLoss = Case When @TotalQualityLoss > (@TotalProduction+@TotalQualityLoss) Then (@TotalProduction+@TotalQualityLoss) Else @TotalQualityLoss End
--*****************************************************
