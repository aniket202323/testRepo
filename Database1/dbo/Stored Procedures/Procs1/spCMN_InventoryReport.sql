Create Procedure dbo.spCMN_InventoryReport
@Unit int,
...
Declare @MaxInventoryDays int
Select @MaxNumberOfRows = 1000
--TODO: Lookup, Or Pass In Inventory Days
Select @MaxInventoryDays = 300
...
Declare @ResultsetNameInventoryList nvarchar(50)
...
Select @ResultsetNameInventoryList = 'Inventory List'
...
Declare @YEnabled 	 Int,@ZEnabled 	 Int,@AEnabled 	 Int
Select @ColumnNameTotalProduction = 'Inventory Amount'
Select @ColumnNameNumberProduction = '#'
Select @ColumnNamePercentProduction = '% Inventory'
If (Select Coalesce(Uses_Start_Time,0) From Prod_Units Where PU_Id = @Unit ) = 0
...
Select @YEnabled = 0,@ZEnabled = 0,@AEnabled = 0
...
select @YEnabled = Dimension_Y_Enabled,@ZEnabled = Dimension_Z_Enabled,@AEnabled = Dimension_A_Enabled,
 	  	  @TempString = s.event_subtype_desc, @TempDimensionX = s.dimension_x_name, @TempDimensionY = s.dimension_y_name, @TempDimensionZ = s.dimension_z_name, @TempDimensionA = s.dimension_a_name
...
If @LanguageNumber <> 0
  Begin
    ...
    Select @TempString = Prompt_String 
      From Language_Data
      Where Language_Id = @LanguageNumber and Prompt_Number = 24251
    Select @ResultsetNameInventoryList = coalesce(@TempString, @ResultsetNameInventoryList)     
    Select @TempString = NULL
    Select @TempString = Prompt_String 
      From Language_Data
    ...
    Select @TempString = Prompt_String 
      From Language_Data
      Where Language_Id = @LanguageNumber and Prompt_Number = 24258
    ...
    Select @TempString = Prompt_String 
      From Language_Data
      Where Language_Id = @LanguageNumber and Prompt_Number = 24260
...
--Do Inventory List After Counting Records In Return
Insert Into #Resultsets (ResultOrder, ResultName)
  Values (2, @ResultsetNameStatusPareto)
Insert Into #Resultsets (ResultOrder, ResultName)
  Values (3, @ResultsetNameProductPareto)
--*********************************************************************************
...
Insert Into #Details
  Select s.icon_id, e.event_num, s.prodstatus_desc, s.status_valid_for_input, e.start_time, e.timestamp,
    ...
    Where e.PU_Id = @Unit and 
          e.Timestamp  >= dateadd(day,-1 * @MaxInventoryDays, @EndTime) and
          e.Timestamp < @EndTime
...
If @NumberOfRows <= @MaxNumberOfRows
  Insert Into #Resultsets (ResultOrder, ResultName)
    Values (1, @ResultsetNameInventoryList)
...
--*********************************************************************************
-- Return Resultset #3 - Inventory List
--*********************************************************************************
If @NumberOfRows <= @MaxNumberOfRows
  Begin
    ...
    Select @SQL = @SQL + 'From #Details  Order By EndTime DESC'
    --print @SQL 
    Execute (@SQL)
  End
--*********************************************************************************
-- Return Resultset #4 - Status Pareto
--*********************************************************************************
...
Execute (@SQL)
--*********************************************************************************
-- Return Resultset #5 - Product Pareto
--*********************************************************************************
