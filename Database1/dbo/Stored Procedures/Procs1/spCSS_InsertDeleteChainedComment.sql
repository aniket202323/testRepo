CREATE PROCEDURE dbo.spCSS_InsertDeleteChainedComment
@KeyId nvarchar(100),
@Table int,
@UserId int,
@CommentSource int,
   ...
   begin
      ...
      update products set Comment_id = @TopOfChainId
        where prod_id = @BIKeyId
 	  	 ...
 	  	 VALUES (1,@UserId,'spCSS_InsertDeleteChainedComment',
 	  	  	  	  Cast(ISNULL(@KeyId,0) as Nvarchar)+','+
 	  	  	  	  	 Cast(ISNULL(@Table,0) as Nvarchar)+','+
 	  	  	  	  	 Cast(ISNULL(@UserId,0) as Nvarchar)+','+
 	  	  	  	  	 Cast(ISNULL(@CommentSource,0) as Nvarchar)+','+
 	  	  	  	  	 Cast(ISNULL(@AddlInfo,'') as Nvarchar)+','+
 	  	  	  	  	 Cast(ISNULL(@AddlInfo2,'') as Nvarchar)+','+
 	  	  	  	  	 Cast(ISNULL(@NewId,0) as Nvarchar)+','+
 	  	  	  	  	 Cast(ISNULL(@CurrentTopOfChainId,0) as Nvarchar) , dbo.fnServer_CmnGetDate(getUTCdate()))
 	 if @@ROWCOUNT = 0 
        begin
          ROLLBACK TRANSACTION
          GOTO Failed
...
ELSE IF @Table = 79 -- NonProductive_Detail Comment (-- mt added:3-16-2006)
 ...
 ELSE IF @Table = 80 --Activities General
    BEGIN
    ...
    BEGIN
      ...
      if @@ROWCOUNT = 0 
        ...
        END
    END
 	 ELSE IF @Table = 89 --ActivityOverdue
    BEGIN
       SELECT @TopOfChainId = CASE
                                           WHEN @Delete = 1
                                           THEN @TopOfChainId
      ...
      if @@ROWCOUNT = 0 
        ...
        END
    END
 	 ELSE IF @Table = 90 --ActivityDetail
    BEGIN
      SELECT @TopOfChainId = CASE
                                                   WHEN @Delete = 1
                                                   THEN @TopOfChainId
