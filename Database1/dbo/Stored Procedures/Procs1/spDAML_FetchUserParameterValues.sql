Create Procedure dbo.spDAML_FetchUserParameterValues
    @ParmId 	  	 INT = NULL,
...
SET @OptionsClause = ''
IF (@UserName<>'' AND @UserName IS NOT NULL) BEGIN
   IF ( CHARINDEX('%', @UserName)=0 AND CHARINDEX('_', @UserName)=0 )
      SET @OptionsClause = @OptionsClause + ' AND u.Username = ''' + CONVERT(VARCHAR(30),@UserName) + ''' '
   ELSE
      SET @OptionsClause = @OptionsClause + ' AND u.Username LIKE ''' + CONVERT(VARCHAR(30),@UserName) + ''' '
END 
...
-- Order clause
SET @OrderClause = ' ORDER BY p.Parm_Name, u.UserName '
