Create Procedure dbo.spDBR_Update_Report_Data
@ReportID int,
@RunTime datetime,
@XML   ntext,
...
Declare @txtXML varchar(max), @IsUnicodeInstalled int  	   
  	    	  SELECT @RunTime = dbo.fnServer_CmnConvertToDBTime(@RunTime,@InTimeZone)
 	  	  SET @IsUnicodeInstalled = 1
IF NOT EXISTS(Select 1 From Appversions where App_name ='Unicode installer'and App_Version =1)
BEgin 
 	 SET @IsUnicodeInstalled = 0;
 	 SET @txtXML = @XML;
End
...
begin
 	 ...
 	 begin
 	  	 insert into dashboard_report_data (Dashboard_report_id, dashboard_time_stamp, dashboard_report_version, dashboard_report_display_name, dashboard_report_xml) 
  	    	    	  values (@ReportID, @RunTime, 1, @ReportTitle, CASE WHEN @IsUnicodeInstalled = 0 then @txtXML Else @XML End)
  	    	    	  ...
  	    	    	  IF @IsUnicodeInstalled = 0
 	  	  	  	 WRITETEXT dashboard_report_Data.dashboard_report_xml @ptrval @txtXML
 	  	  	  Else
 	 ...
 	 begin
 	  	 ...
 	  	 begin 	  	  	 
 	  	  	 ...
 	  	  	 insert into dashboard_report_data (Dashboard_report_id, dashboard_time_stamp, dashboard_report_version, dashboard_report_display_name, dashboard_report_xml) 
  	    	    	  values (@ReportID, @RunTime, @ThisVersion, @ReportTitle, CASE WHEN @IsUnicodeInstalled = 0 then @txtXML Else @XML End)
