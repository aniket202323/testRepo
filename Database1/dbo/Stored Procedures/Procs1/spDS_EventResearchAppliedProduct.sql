Create Procedure dbo.spDS_EventResearchAppliedProduct
 @EventId int
AS
 Declare @TimeStamp datetime
 Declare @PUId int
 Select @TimeStamp = TimeStamp, @PUId = PU_Id
  From Events
 ...
 If @TimeStamp Is Null Return
   ...
   Into #SpecsWithTests
    From var_specs vs
     Join #Tests t on t.var_id = vs.var_id 
      Where vs.Effective_date <= @TimeStamp and ((vs.expiration_date > @TimeStamp) 
  ...
  Select Distinct s.prod_id, p.prod_code , Prod_Desc
