CREATE PROCEDURE [dbo].[spDowntime_SplitDownTime]
...
BEGIN
    ...
    /*
     Verifying split permissions
     */
    DECLARE @MasterUnit_Security Int, @LocationId Int, @UsersSecurity Int, @ActualSecurity Int
    Select @LocationId = AssetId from @inputDowntimeTable where Id = 1 -- selecting one of the unitIds. All of the unitIds needs to be same
    -- TODO put validations that unitIds in all the records should be same
    SELECT @MasterUnit_Security = COALESCE(Master_Unit,@LocationId) FROM Prod_Units_Base WHERE PU_Id = @LocationId
    SELECT @UsersSecurity = dbo.fnBF_CmnGetUserAccessLevel(@MasterUnit_Security,@UserId,1)
    -- 395 display option Id for splitDowntime security, default access level = 3 [manager], dt classic display Optionid = NULL
    -- We will always get this option as 1 for dt classic sheet
    SELECT @ActualSecurity = dbo.fnCMN_CheckSheetSecurity(@MasterUnit_Security,NULL,395,3,@UsersSecurity)
    IF @ActualSecurity = 0
        BEGIN
            SELECT Code = 'InsufficientPermission', Error = 'InsufficientPermission - Split Permission Not available', ErrorType = 'InsufficientSplitPermission', PropertyName1 = '', PropertyName2 = '', PropertyName3 = '', PropertyName4 = '', PropertyValue1 = '', PropertyValue2 = '', PropertyValue3 = '', PropertyValue4 = ''
            RETURN
        END
        ...
        WHILE (@CurrentRow <=@MaxRow)
            BEGIN
                ...
                SELECT @tedet_id = TedDetId,@Code = code, @Error =error,@ErrorType = ErrorType, @PropertyName1 = PropertyName1,
                       ...
                       @PropertyValue4 = PropertyValue4, @RecordChanged = RecordChanged FROM  fnDowntime_ModifyDowntime_Validations(
                        @TransType
                    ,@TEDetId
                    ,@StartTime
                    ,@EndTime
                ...
                IF @RecordChanged=1
                    BEGIN
                        ...
                        ELSE
                            BEGIN
                                SELECT
                                        @CurrentCommentId = @CauseCommentId
                                     ,@CurrStatus = NULL
    ...
    SELECT TEDet_Id AS 'DetailId',
           ...
           r1.Event_Reason_Name AS 'Reason1', r1.Comment_Required as Comment_Required_Reason1,
           ted.reason_level2 AS 'Reason2Id',
           r2.Event_Reason_Name AS 'Reason2', r2.Comment_Required as Comment_Required_Reason2,
           ted.reason_level3 AS 'Reason3Id',
           r3.Event_Reason_Name AS 'Reason3', r3.Comment_Required as Comment_Required_Reason3,
           ted.reason_level4 AS 'Reason4Id',
           r4.Event_Reason_Name AS 'Reason4', r4.Comment_Required as Comment_Required_Reason4,
           ted.Action_level1 AS 'Action1Id',
           a1.Event_Reason_Name AS 'Action1', a1.Comment_Required as Comment_Required_Action1,
           ted.Action_level2 AS 'Action2Id',
           a2.Event_Reason_Name AS 'Action2', a2.Comment_Required as Comment_Required_Action2,
           ted.Action_level3 AS 'Action3Id',
           a3.Event_Reason_Name AS 'Action3', a3.Comment_Required as Comment_Required_Action3,
           ted.Action_level4 AS 'Action4Id',
           a4.Event_Reason_Name AS 'Action4', a4.Comment_Required as Comment_Required_Action4,
