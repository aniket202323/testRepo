CREATE PROCEDURE dbo.spEM_CreateEventReason
...
Insert into Audit_Trail(Application_Id,User_id,Sp_Name,Parameters,StartTime)
 	 VALUES (1,@User_Id,'spEM_CreateEventReason',
...
select @Insert_Id = Scope_Identity()
  ...
  If Exists (select * from dbo.syscolumns where name = 'Event_Reason_Name_Local' and id =  object_id(N'[Event_Reasons]'))
 	 If (@@Options & 512) = 0
 	   ...
 	   End
  COMMIT TRANSACTION
  Update  Audit_Trail set EndTime = dbo.fnServer_CmnGetDate(getUTCdate()),returncode = 0,Output_Parameters = convert(nVarChar(10),@EventReason_Id) where Audit_Trail_Id = @Insert_Id
  RETURN(0)
