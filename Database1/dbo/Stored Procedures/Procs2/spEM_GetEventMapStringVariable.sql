CREATE PROCEDURE [dbo].[spEM_GetEventMapStringVariable]
  @MU 	 Int
  AS
DECLARE   @ETId 	  	 Int,
          @PreviousETId Int,
...
DECLARE @EventSubtype1 Nvarchar(max)
DECLARE @EventStr Nvarchar(max)
DECLARE @EventSubtypeDimensions Nvarchar(maX)
Declare @EventSubtype nvarchar(max)
...
DECLARE ECS Cursor
      ...
      Begin
        If @ETID = 17
          Begin
 	  	  	 ...
 	  	  	 IF charindex(@SubType,@EventSubtype1) = 0
            Select @EventSubtype1 = @EventSubtype1 +  @SubType
            GoTo ESTLoop
          End
        Else
          Begin
 	  	    
            Select @EventSubtype1 = @EventSubtype1 + Char(3) +  Convert(nVarChar(10),@ETID) + Char(4) + Event_Subtype_Desc  From event_subtypes Where Event_Subtype_Id = @ESID
            GoTo ESTLoop
          End
      End
    Close EST
    ...
    Close ES
    Deallocate ES
 	 SELECT @EventSubtype = @EventSubtype1
 	 INSERT INTO @MasterUnits
