...
CREATE PROCEDURE dbo.spEM_IEGetExportData 
...
ELSE If @DataType = 'DisplayVariables'
 	 BEGIN
 	   SELECT @Sql = 'SELECT s.Sheet_Desc,pl.PL_Desc,pu.PU_Desc,v.Var_Desc,sv.Title,sv.Var_Order,sv.Activity_Order,sv.Execution_Start_Duration,sv.Target_Duration,sv.Activity_Alias,sv.AutoComplete_Duration,sv.External_URL_link,sv.Open_URL_Configuration,sv.[Password],sv.User_Login '
...
BEGIN
 	 SELECT Path_Code,Prod_Code
 	  	 ...
 	  	 WHERE (pep.PL_Id = @Id and @QueryType = 2 ) OR (pep.PL_Id in (select Pl_id from Prod_Lines_base where dept_Id = @Id) and @QueryType = 1)
 	  	 ORDER BY Path_Code
END 	 
...
BEGIN
 	 ...
 	 INSERT INTO  @euPU
 	  	 ...
 	  	 JOIN Prdexec_Paths p on p.path_Id = pu.Path_Id and 
 	  	 --p.PL_Id = @Id
 	  	 ((p.PL_Id = @Id and @QueryType = 2 ) OR (p.PL_Id in (select Pl_id from Prod_Lines_base where dept_Id = @Id) and @QueryType = 1))
