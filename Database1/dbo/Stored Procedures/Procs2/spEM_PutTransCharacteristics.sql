CREATE PROCEDURE dbo.spEM_PutTransCharacteristics
  @Trans_Id int,
  @PU_Id     int,
  @Prod_Id   int,
  @Prop_Id   int,
...
DECLARE @Insert_Id Integer, 
  ...
  INSERT INTO Audit_Trail(Application_Id,User_id,Sp_Name,Parameters,StartTime)
 	 VALUES (1,@User_Id,'spEM_PutTransCharacteristics',
  ...
  --
  Select @Id = Null
  SELECT @Id = Trans_Id FROM Trans_Characteristics
    WHERE (Trans_Id = @Trans_Id) AND (PU_Id = @PU_Id) AND (Prod_Id = @Prod_Id) AND (Prop_Id = @Prop_Id)
  Select @CharExistsId = Null
  Select @CharExistsId = Char_Id From pu_Characteristics Where(PU_Id = @PU_Id) AND (Prod_Id = @Prod_Id) AND (Prop_Id = @Prop_Id)
  --
  ...
  ELSE
    ...
    End
  --
  -- Delete any related transactions in the trans_propertys and trans_variables
  --
Delete From Trans_Variables Where Trans_Id = @Trans_Id and Var_Id in (Select Var_Id From Variables Where Spec_Id in(Select Spec_Id From Specifications Where Prop_Id = @Prop_Id))
Delete From Trans_Properties Where Trans_Id = @Trans_Id  and Char_Id = @Char_Id and Spec_Id in (Select Spec_Id From Specifications Where Prop_Id = @Prop_Id)
  --
  -- Return success.
  --
