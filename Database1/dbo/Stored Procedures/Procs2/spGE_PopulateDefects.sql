CREATE Procedure dbo.spGE_PopulateDefects
 	 @EventId Int
As
Declare @ParentEvent Int
/*
To Do
Figure out who is parent (currently parent of parent)
*/
...
--Delete From #ChildDefects Where Event_Id <> @EventId
--select * from #ChildDefects
--select * from #Defects
If (Select count(*) from #Defects) > 0
Select Defect_Id,c.Event_Id,Final_Dimension_Y,Start_X ,End_X,Start_Y,End_Y
 from #ChildDefects c
-- Join Event_Components ec on ec.source_Event_Id = c.Event_Id
-- Join Event_Components ec on ec.source_Event_Id = @ParentEvent
...
Union
Select Defect_Id= Defect_Detail_Id,Event_Id,Final_Dimension_Y = 0,Start_X = Start_Position_X,
 	 End_X = End_Position_X,Start_Y = Start_Position_Y,End_Y = End_Position_Y
 from Defect_Details
