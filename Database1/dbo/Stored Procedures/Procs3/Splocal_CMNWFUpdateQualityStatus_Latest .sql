...
CREATE PROCEDURE   [dbo].[Splocal_CMNWFUpdateQualityStatus_Latest ]
...
-- Get the Packing lot storage unit
	...
	SET @Batchpuid            =  (SELECT	PU_ID	FROM	dbo.Prod_Units   WITH(NOLOCK)	WHERE	Equipment_Type = @EquipTypeLotStorage)
...
Select '@Tabledata', * from @Tabledata
...
(PRodid,Product,Batchcode,Postingdate,DocuemntDate,ReferenceNumber,Location,MovementType,MovementTypedesc,Quantity,UOM,InsertedDate,OrderStartdate,OrderFinishdate,FromStatus,ToStatus)
 SELECT @prodid,prodcode, Batchcode,Postingdate,Documentdate,ReferenceNumber,Location,MovementType,MovementTypeDesc,Quantity,UOM,Getdate(),OrderStartdate,Orderfinishdate,@FromEventStatus,@UpdateStatusdesc FROM @Tabledata
