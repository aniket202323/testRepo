...
EXECUTE [spLocal_CTS_Get_Appliances_by_Criteria] NULL, '11859,11858',NULL, NULL, NULL,'Bergeron.fe'

SELECT * FROM event_details WHERE event_id = 997982
*/

CREATE PROCEDURE [dbo].[spLocal_CTS_Get_Appliances_by_Criteria_test]
...
BEGIN
	...
	(
		...
		Appliance_status_desc							VARCHAR(25),
	...
	DECLARE 
	@FApplianceLocations TABLE
	(
		PU_Id						INTEGER,
		PU_Desc						VARCHAR(50)
	)

	INSERT INTO @FApplianceLocations(PU_DESC)
		SELECT value FROM STRING_SPLIT(@F_Appliance_location, ',');

	UPDATE	@FApplianceLocations
	SET		PU_Id = PUB.PU_Id 
	FROM	@FApplianceLocations FAL
			JOIN dbo.prod_units_base PUB
				ON FAL.pu_desc = PUB.PU_desc 
	
		
		IF (SELECT COUNT(1) FROM @FApplianceLocations)>0
			INSERT INTO @FAppliance_locations (PU_ID, PU_Desc)
				SELECT	PUB.PU_ID, PUB.PU_Desc
				FROM	dbo.Prod_Units_Base PUB WITH(NOLOCK) 
						 JOIN @FApplianceLocations  FAL
							ON FAL.PU_Desc = PUB.PU_Desc
	...
	IF (SELECT event_id FROM dbo.event_details WITH(NOLOCK) WHERE alternate_event_num = @Serial)  IS NOT NULL
	BEGIN
		...
		SELECT	EDAPP.Alternate_event_num 'Serial',
				...
				ELTRANS.Event_Status 'Apliance_status_Id',
				(SELECT prodStatus_Desc FROM dbo.production_status WITH(NOLOCK) WHERE prodstatus_id = ELTRANS.Event_Status) 'Apliance_status_Desc',
				APPROD.Last_product_id 'Appliance_Product_Id',
				APPROD.Prod_Desc 'Appliance_Product_desc',
		...
		FROM	dbo.Prod_Units_Base PUB WITH(NOLOCK)
			...
			-- PROCESS ORDER
				...
				OUTER APPLY (SELECT last_product_id,PB.prod_desc FROM fnLocal_CTS_Appliance_Status(EAPP.event_id) JOIN dbo.products_base PB ON PB.prod_id = last_product_id) APPROD
		...
		WHERE EAPP.event_id = (SELECT event_id FROM dbo.event_details WITH(NOLOCK) WHERE alternate_event_num = @Serial)
	
	END
	ELSE
	BEGIN

	-- ALL Appliances, last cleaning, actual status, current reservation and last production
		SELECT	EDAPP.Alternate_event_num 'Serial',
				EAPP.event_id 'Appliance_Id',
				EAPP.event_num 'Appliance _desc',
				FAT.type 'Appliance_Type',
		...
		FROM	dbo.Prod_Units_Base PUB WITH(NOLOCK)
				...
				OUTER APPLY (SELECT TOP 1 * FROM dbo.event_components EC WITH(NOLOCK) WHERE EC.Source_Event_Id = EAPP.Event_id ORDER BY timestamp DESC) AS ECLTRANS -- WHERE THE APPLIANCE IS NOW
				OUTER APPLY (SELECT * FROM fnLocal_CTS_Appliance_Status(EAPP.event_id) JOIN dbo.products_base PB ON PB.prod_id = last_product_id) APPROD
				LEFT JOIN dbo.events ELTRANS WITH(NOLOCK) ON ELTRANS.Event_Id = ECLTRANS.Event_Id
				LEFT JOIN dbo.Prod_Units_Base PULTRANS WITH(NOLOCK) ON PULTRANS.PU_id = ELTRANS.PU_Id
				LEFT JOIN @FAppliance_locations FAL ON FAL.pu_id = ELTRANS.pu_id

			-- LAST OR ACTIVE CLEANING
				OUTER APPLY 
				(	
				SELECT	Status,
						Type,
				...
				--		timer_exception	
				--FROM	fnLocal_CTS_Appliance_Status(997935)
				--) AS APPS
			...
			-- PROCESS ORDER
				OUTER APPLY(SELECT TOP 1 * FROM dbo.production_plan_starts PPS WHERE PPS.PP_ID = APPROD.Last_Process_order_Id ORDER BY PPS.start_time DESC) AS POSINFO
				LEFT JOIN dbo.production_plan POINFO ON POINFO.PP_ID = POSINFO.PP_ID 
			/*
				LEFT JOIN dbo.products_Base POP WITH(NOLOCK) ON POP.Prod_Id = POINFO.Prod_Id 
					AND POP.Prod_Id IN(SELECT Product_Id FROM @FProducts)
...
--	Return
		...
		FROM	dbo.Prod_Units_Base PUB WITH(NOLOCK)
				...
				OUTER APPLY (SELECT * FROM fnLocal_CTS_Appliance_Status(EAPP.event_id) JOIN dbo.products_base PB ON PB.prod_id = last_product_id) APPROD
				LEFT JOIN dbo.events ELTRANS WITH(NOLOCK) ON ELTRANS.Event_Id = ECLTRANS.Event_Id
				OUTER APPLY (SELECT * FROM fnLocal_CTS_Location_Status(ELTRANS.pu_id) JOIN dbo.products_base PB ON PB.prod_id = last_product_id) LOCROD
				LEFT JOIN dbo.Prod_Units_Base PULTRANS WITH(NOLOCK) ON PULTRANS.PU_id = ELTRANS.PU_Id
				LEFT JOIN @FAppliance_locations FAL ON FAL.pu_id = ELTRANS.pu_id
			-- CLEANING
				OUTER APPLY 
			...
			-- RESERVATION
				...
				-- PROCESS ORDER
				-- ALL UNITS OF THE PATH STARTS SIMULTANOUSLY
				OUTER APPLY(SELECT TOP 1 * FROM dbo.production_plan_starts PPS WHERE PPS.PP_ID = LOCROD.Last_Process_order_Id ORDER BY PPS.start_time DESC) AS POSINFO
				LEFT JOIN dbo.production_plan POINFO ON POINFO.PP_ID = POSINFO.PP_ID 
				LEFT JOIN dbo.products_Base POP WITH(NOLOCK) ON POP.Prod_Id = POINFO.Prod_Id 
					AND POP.Prod_Id IN(SELECT Product_Id FROM @FProducts)
				LEFT JOIN dbo.Production_Plan_Statuses PPSt WITH(NOLOCK)
...
GRANT EXECUTE ON [dbo].[spLocal_CTS_Get_Appliances_by_Criteria] TO ctsWebService
GRANT EXECUTE ON [dbo].[spLocal_CTS_Get_Appliances_by_Criteria] TO comxclient
