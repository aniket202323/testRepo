...
--------------------------------------------------------------------------------------------------
-- Stored Procedure: spLocal_CmnCalcVBConsumptionRawMaterial_FO02809
--------------------------------------------------------------------------------------------------
-- Author				:	Ugo Lapierre
...
-- 1.29			2016-11-01		U.Lapierre				Fix issue when carry over occurs.  Non active pallet.
-- 2.0			2016-11-10		Julien B. Ethier		New version for SCO project
-- 2.01			2016-11-21		L.Hudon					never consumed on Returned pallets
-- 2.02			2016-12-15		U.Lapierre				Fix an issue with consumption type = 3 where BOM and RMI scrap factor not used correctly
-- 2.03			2017-02-21		U.Lapierre				FO-02856.  Remove the exit clause when PO start_time!=event start_time if the path UDP PE_General_DisableProductionEvent is TRUE (support multiple event per PO)
-- 2.04			2017-04-21		Julien B. Ethier		Fixed sp to prevent "Invalid result set row error in CalculationMgr log file
-- 2.05			2017-05-31		U.Lapierre				Fix carry over issue when getting ownership of the PP_ID...
-- 2.1			2017-05-31		Julien B. Ethier		FO-02809. Adapted sp to support bulk material
--================================================================================================
--
/*

declare @OutputValue varchar(50)
Exec dbo.spLocal_CmnCalcVBConsumptionRawMaterial_FO02809
		@OutputValue		OUTPUT,
		@PUId				= 3888,				
		@TimeStamp			= '2017-05-24 15:15:59',
		@DefaultUserName	= 'PE.Consumption',		
		@ProdCount			= 5385,
...
-------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[spLocal_CmnCalcVBConsumptionRawMaterial_FO02809]
...
DECLARE	@pnMaterialOriginGroup		varchar(50),
		...
		@IsSCOLine					bit,
		@TableIdPath				int,
		@DisableCreateEventUDP		int,			--V2.03
		@DisableCreateEventValue	bit	
...
SELECT	@OutputValue = 'DONOTHING',
		...
		SET @spName = 'spLocal_CmnCalcVBConsumptionRawMaterial_FO02809'
-------------------------------------------------------------------------------
-- Debug Setting
...
SET @RMReturnStatusId = NULL
SET	@RMReturnStatusId	=(SELECT ProdStatus_Id
							FROM dbo.Production_Status WITH(NOLOCK)
							WHERE ProdStatus_Desc = 'Returned')
...
SET @udpIsSCOLineId = (	SELECT Table_Field_Id 
							FROM dbo.Table_Fields 
							WHERE Table_Field_Desc = 'PE_General_IsSCOLine'
								AND tableid = @TableIdPath)

SET @IsSCOLine	=  (	SELECT value
							FROM dbo.Table_Fields_Values
							WHERE Table_Field_Id = @udpIsSCOLineId
								AND keyid = @pathId
								AND tableid = @TableIdPath
								 )	
...
--V2.03 get UDP to enable/disable creation of production event
SET @DisableCreateEventUDP = (	SELECT table_field_ID 
								FROM dbo.table_fields  WITH(NOLOCK) 
								WHERE Table_Field_Desc = 'PE_General_DisableProductionEvent'
									AND tableid = @TableIdPath)

SET @DisableCreateEventValue = (	SELECT CONVERT(BIT,Value)
							FROM Table_Fields_Values WITH(NOLOCK) 
							WHERE table_field_id = @DisableCreateEventUDP
								AND keyid = @pathId)

IF @DisableCreateEventValue IS NULL
	SET @DisableCreateEventValue = 0



		IF @DebugFlagManual = 1
			BEGIN
				SELECT @DisableCreateEventUDP as 'UDP disable', @DisableCreateEventValue, @TableIdPath as 'path id'
			END
...
IF @DebugFlagOnLine = 1  
	INSERT intO Local_Debug(Timestamp, CallingSP, Message) 
		VALUES(	getdate(), 
				...
				' PUId=' + convert(varchar(5), @PUId) + 
				' TimeStamp=' + convert(varchar(25), @TimeStamp, 120) + 
				' @DisableCreateEventValu e= ' + convert(varchar(25), @DisableCreateEventValue, 120) 
			)
...
------------------------------------------------------------------------------------------------------
-- Verify there is a product attached to the PO.
------------------------------------------------------------------------------------------------------
SET		@ProdCode = NULL
...
--IF @IsSCOLine = 0 removed in V2.03
IF @IsSCOLine = 0 AND @DisableCreateEventValue = 0  --V2.03
BEGIN
	...
	BEGIN
			...
			INSERT INTO @LogDetails (LogDetailId)
...
SET	IsBulkMaterial = CASE es.Event_Subtype_Desc
							WHEN 'Bulk_Unit' THEN 1
							ELSE 0
							END
FROM @tblBOMRMListComplete	brmc
JOIN dbo.Event_Configuration	ec WITH(NOLOCK) ON ec.PU_Id = brmc.BOMRMStoragePUId
...
IF @DebugFlagManual = 1
	BEGIN
		SELECT '@tblRMListComplete-IsBulkMaterial', * FROM @tblBOMRMListComplete
	END
...
/*New UDPs*/
		...
		-- For bulk material, we check which of the storage tanks contains the
		-- lot that has been checked-in


		IF @IsBulkMaterial = 1
		BEGIN
			IF EXISTS(SELECT * 
						FROM @tblRMParentInfo rm
						JOIN dbo.Events e			WITH(NOLOCK) ON e.PU_Id = rm.ParentPUId
						JOIN dbo.Event_Details ed	WITH(NOLOCK) ON ed.Event_Id = e.Event_Id
						WHERE ParentId = @LoopIndex
							  AND ed.PP_Id = @PPId)
			BEGIN
				SET @IsCurrentParent = 1
				...
				BEGIN
					INSERT intO Local_Debug(Timestamp, CallingSP, Message) 
						VALUES(	getdate(), 
								@spName,
								'0335' +
								' @IsCurrentParent=' + convert(varchar(1), COALESCE(@IsCurrentParent, 0)) +
								' @PEIId=' + convert(varchar(5), @PEIId) + 
								' @BOMRMStoragePUId=' + convert(varchar(5), @BOMRMStoragePUId) + 								
								' @BOMRMStoragePUDesc=' + convert(varchar(50), @BOMRMStoragePUDesc))								
				END
		...
		-- Update the BOM list based on which bulk lot is checked-in.
		IF @IsCurrentParent IS NULL
		BEGIN
			UPDATE @tblBOMRMListComplete
		...
		BEGIN
			...
			IF @DebugFlagManual = 1
			BEGIN
				SELECT '@IsCurrentParent-test', @PEIId, @BOMRMStoragePUId, @BOMRMStoragePUDesc
				SELECT '@tblBOMRMListComplete-test', * FROM @tblBOMRMListComplete				
			END
			...
			IF @DebugFlagManual = 1
			BEGIN
				SELECT '@tblBOMRMListComplete:FlgInputToThisUnit', * FROM @tblBOMRMListComplete
			END
...
SET		PEIId					= rmpi.PEIId,
		...
		IsBulkMaterial			= rmpi.IsBulkMaterial
FROM @tblBOMRMListComplete	brmc
JOIN @tblRMParentInfo		rmpi ON brmc.BOMRMProdGroupDesc = rmpi.BOMRMProdGroupDesc
									AND (brmc.BOMRMStoragePUId = rmpi.ParentPUId
									OR	rmpi.IsCurrentParent = 1)
									

-- Update bulk material storage PU information
UPDATE @tblBOMRMListComplete
SET		BOMRMStoragePUId		= rmpi.ParentPUId,
		BOMRMStoragePUDesc		= rmpi.ParentPUDesc
FROM @tblBOMRMListComplete	brmc
JOIN @tblRMParentInfo		rmpi ON brmc.BOMRMProdGroupDesc = rmpi.BOMRMProdGroupDesc
									AND rmpi.IsCurrentParent = 1


IF @DebugFlagManual = 1
	BEGIN
		SELECT '@tblBOMRMListComplete', * FROM @tblBOMRMListComplete
	END
...
--New  1.21  --get possession of pallet only if your PO staill active
IF EXISTS(SELECT pp_id FROM dbo.production_plan WHERE pp_id = @PPId AND pp_status_id = 3)  AND @IsSCOLine = 0   --V2.05   (I added SCO FALSE condition and COMPLETE)
BEGIN
	UPDATE ed
	SET pp_ID = @PPId
...
BEGIN
	...
	--SELECT @IsProdCounterPOBOMQty = BOMRMQty  --remove in 2.02
	SELECT @IsProdCounterPOBOMQty = OrBOMRMQty  --2.02
		FROM @tblBOMRMList
...
IF @DebugFlagManual = 1
	BEGIN
		SELECT	'@IsProdCounterPOBOMQty', @IsProdCounterPOBOMQty
	END
...
BEGIN
	...
	BEGIN
			INSERT intO Local_Debug(Timestamp, CallingSP, Message) 	
			VALUES(	getdate(), @spName,
			'@PPId = ' +  CONVERT(Varchar(50),@PPId) +
			' @PEIId = ' +  CONVERT(Varchar(50),@PEIId) +
			' @FlgReportAsConsumption = ' +  CONVERT(Varchar(50),@FlgReportAsConsumption) +
			' @BOMRMProdId = ' +  CONVERT(Varchar(50),@BOMRMProdId) +
			' @ScannerCheckedInFlag = ' +  CONVERT(Varchar(50),@ScannerCheckedInFlag) 
			)

	END	

	--V2.05 - Get possession of Pallet that are on the proper units, but wrong PPID or PPID NULL (SCO only)
	IF @IsSCOLine = 1 AND @EventStatusStr <> 'Complete'
	BEGIN
		--Get PP_ID from production_plan_starts
		SET @ppid = (	SELECT pp_id 
						FROM dbo.production_plan_starts 
						WHERE pu_id = @puid
							AND start_time<@timestamp AND (end_time IS NULL OR end_time>=@timestamp))
		...
		JOIN dbo.prdExec_input_Sources peis	WITH(NOLOCK)	ON e.pu_id = peis.pu_id 
															AND peis.pei_id = @PEIId
		JOIN dbo.Production_Status ps		WITH(NOLOCK)	ON (ps.ProdStatus_Id = e.Event_Status)		
		WHERE	(	(ps.Count_For_Production = 0 	AND ps.Count_For_Inventory = 0)
					OR	 (ps.Count_For_Production = 1 	AND ps.Count_For_Inventory = 1 AND ed.final_dimension_X > 0)
				)
		AND		@FlgReportAsConsumption = 1
		AND     @ScannerCheckedInFlag = 0
		AND		(ed.pp_ID != @PPId OR ed.pp_ID IS NULL)
		AND  (e.applied_product = @BOMRMProdId
				OR e.applied_product = @BOMRMSubProdId)	
		AND		@IsBulkMaterial = 0		
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				--SET @AdjustedProdCount = @ProdCount + (@ProdCount*(@BOMRMScrapFactor/100))  Removed in 2.02
				SET @BOMRMQty = @BOMRMQty + (@ORBOMRMQty*(@BOMRMScrapFactor/100))
			END
			ELSE
			BEGIN
		...
		BEGIN		
			...
			--SET @RMTotConsumedQty =  CONVERT(DECIMAL(18,6), @AdjustedProdCount) --Remove in 2.02
			SET @RMTotConsumedQty =  CONVERT(DECIMAL(18,6), @ProdCount / @IsProdCounterPOBOMQty * @BOMRMQty)  --2.02			
		END
		ELSE
		BEGIN			
			IF @DebugFlagManual = 1
...
SET @RMTotConsumedQtyInactive = (SELECT coalesce(sum(coalesce(Dimension_X,0)),0)
										FROM dbo.Event_Components ec WITH(NOLOCK)
										JOIN dbo.events rmi ON (rmi.Event_Id = ec.Source_Event_Id)
										WHERE rmi.event_id NOT IN (SELECT RMeventid FROM @tblRMInventorytmp)
											AND ec.Event_Id = @EventId
											AND rmi.applied_product IN (@BOMRMProdId,@BOMRMSubProdId)
											)
...
SET @RMTotConsumedQtyActiveExisted = (SELECT coalesce(sum(coalesce(Dimension_X,0)),0)
										FROM dbo.Event_Components ec WITH(NOLOCK)
										JOIN dbo.events rmi ON (rmi.Event_Id = ec.Source_Event_Id)
										WHERE rmi.event_id IN (SELECT RMeventid FROM @tblRMInventorytmp)
											AND ec.Event_Id = @EventId
											AND rmi.applied_product IN (@BOMRMProdId,@BOMRMSubProdId)
											)
...
END											
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				VALUES(	getdate(), 
						@spName,
						'Active Pallet Genealogy Link Not Existed : Create the link');
			END
	...
	--AND	e.Event_Status <> @RMReturnStatusId
		--ORDER BY ec.ENTRY_ON DESC
		ORDER BY e.timestamp DESC

		IF ((SELECT Event_Status FROM dbo.Events WITH(NOLOCK) WHERE Event_ID =@RMEventId) = @RMReturnStatusId)
		BEGIN
			SET @RMEventId = NULL
		END
