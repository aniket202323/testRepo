...
-- SP Type				: Calculation
-- Caller				: 
-- Description			:  Simulation for PrIME.  Change Open request status to Delivered (1 pallet per minute max)
...
--------------------------------------------------------------------------------------------------
--================================================================================================
-- 1.0		2021-01-15		Julien B. Ethier		Created from PE's spLocal_MPWS_CmnPrIMESimulationSetDelivered
--------------------------------------------------------------------------------------------------

CREATE PROCEDURE [dbo].[spLocal_MPWS_CmnPrIMESimulationSetDelivered]
...
Exec spLocal_MPWS_CmnPrIMESimulationSetDelivered
	@OutputValue				OUTPUT,
	'15-Aug-2019 15:25',				
	5812
...
SET @SPNAME = 'spLocal_MPWS_CmnPrIMESimulationSetDelivered'
SELECT @Now = GETDATE()
SET @Count = 0
...
SET @WMSid = (	SELECT min([OpentableId]) 
				...
				WHERE LocationID = @LocationID AND Status = 'InTransit/Waiting')
...
SELECT	@OutputValue =  CONVERT(varchar(30),@count) + ' Stack Delivered'
