...
/*	-------------------------------------------------------------------------------
	dbo.spLocal_MPWS_PLAN_ContainerExitKanban_WIP
		
	This sp returns header info for [spLocal_MPWS_PLAN_ContainerExitKanban_WIP]
...
CREATE PROCEDURE [dbo].[spLocal_MPWS_PLAN_ContainerExitKanban_WIP]
...
SELECT @ConEvent_Id = e.Event_Id, @ConPU_Id = e.PU_Id, @ConProdDesc = p.Prod_Desc, @ConWeight = Final_Dimension_X 
...
ELSE
	BEGIN
		...
		IF (@ConKanbanLoc IS NULL)
			BEGIN
				SET @Message = 'Error: Container not in a Kanban '
			END
		ELSE
			BEGIN
				SELECT @KanPU_Id = PU_Id
					FROM Prod_Units_Base
					WHERE PU_Desc = @ConKanbanLoc
				IF (@KanPU_Id IS NULL)
					BEGIN
									...
									BEGIN
										...
										ELSE
											BEGIN			
												...
												SELECT @Quantity = Result, @KanQtyFloat = CONVERT(FLOAT, Result),
														@KanTestId = Test_Id,
												...
												SET @EntryOn = GETDATE()
												IF (@KanQtyFloat IS NULL OR @KanQtyFloat <= 1)
												SET @KanQtyFloat = 1
												SET @KanQtyChar = CONVERT(VARCHAR(25), @KanQtyFloat - 1)
												...
												--DECLARE @UseByDate AS DATETIME
												--EXEC spLocal_MPWS_DISP_CheckUseByDate @Message, @UseByDate, @Container

												SET	@Message = @Container + ' removed from Kanban'
