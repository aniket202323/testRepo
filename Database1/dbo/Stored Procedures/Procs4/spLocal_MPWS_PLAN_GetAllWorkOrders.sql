...
/*	-------------------------------------------------------------------------------
	...
	This sp returns header info for [spLocal_MPWS_PLAN_GetAllWorkOrders]
...
CREATE PROCEDURE [dbo].[spLocal_MPWS_PLAN_GetAllWorkOrders]
...
BEGIN
	...
	UPDATE @WO_Local SET MaterialName = @MaterialName,
							...
							TargetQty = @TargetQty, 
							UpperLimit = CAST(CAST(@UpperLimit as DECIMAL(10,3)) AS VARCHAR(25)), 
							LowerLimit = CAST(CAST(@LowerLimit as DECIMAL(10,3)) AS VARCHAR(25)),
							UOM = @UOM,
...
IF @Filter IS NULL
	SET @Filter = ''
SELECT WorkOrder, MaterialName, GCASNumber, DispenseNum, TargetQty, UpperLimit, LowerLimit, UOM, DispensedQty, Status, DispenseStation from @WO_Local WHERE Status NOT LIKE 'Cancelled' AND Status NOT LIKE 'Canceled' AND Status NOT LIKE 'Dispensed' AND (GCASNumber LIKE '%'+@Filter+'%')
