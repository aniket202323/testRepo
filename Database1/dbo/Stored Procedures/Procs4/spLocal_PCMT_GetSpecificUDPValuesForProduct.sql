...
CREATE PROCEDURE [dbo].[spLocal_PCMT_GetSpecificUDPValuesForProduct]
/*
*****************************************************************************************************************
Stored Procedure:		spLocal_PCMT_GetSpecificUDPValuesForProduct
...
SET @intTableId = (SELECT tableid FROM dbo.tables WHERE tablename = 'Products')

INSERT #IDENTITY (Item_Desc, Var_Id)
	SELECT DISTINCT tfv.value, @intProdId
...
EXEC sp_executesql @vcrSQLCommand, @vcrSQLParams, @intTableFieldId, @intProdId, @intTableId
