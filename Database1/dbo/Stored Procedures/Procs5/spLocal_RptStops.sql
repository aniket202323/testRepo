...
-- Version 6  
--  
-- This SP will support the report template called RptStopsSummary.xlt.  This  
-- report returns the Stops records for a collection of Production Lines and  
-- allows filters for Products, Categories, etc.  It returns summarized data in  
-- 7 different tabs.  In addition to those 7 resultsets, this SP will also return  
-- an ErrorMsg resultset and a Filter summary resultset.  
--  
-- 2002-04-28 Brent Seely Original  
--  
-- 2002-05-02   Vince King  
--  - Added Comment to the result set.  Added Comment column to the  
--    temporary table, #Delays.  Added code to update #Delays with  
--    comments linked to the Timed_Event_Details and the   
--     Timed_Event_Summarys tables.  
--  - Added Fault Description to the result set.  
--  - Reordered the columns in the final result set Select statement.  
--  - Modified Stops definitions to Not include events that started  
--    prior to the report start time.  This was set for all stops   
--    columns, Stops, Minor Stops, Breakdown, etc.  
--  
-- 2002-05-03   Vince King  
--  - Added Line Status to the result set.  
--   - Added the code for Summary = 1.  This provides the additional  
--    result sets for each of the groups (Team, Shift, Product,  
--     Location Type, Category, and Schedule).  
  
-- 2002-05-15  Vince King  
--  - Added Total Time column to the first result set.  The Total Time  
--    is by line and will be used in the MTBF, MTTR, and R(2) calcs  
--     in the pivot table.  
--  
-- 2002-05-31  Vince King  
--  - Added functionality to retrieve the parent roll Ids for converting lines.    
--    Parameters: Added intRptMaxNoOfUWS, strRptCvtgDTPUStr, strRptPRIDVarStr, strRptUWSVarStr  
--  
-- 2002-06-19 Vince King  
--  - Modified so that the new report parameters added for PRID (@MaxNoOfUWS, @CvtgDTPUStr,  
--  - @PRIDVarStr, @UWSVarStr) values are not required.  
--  
-- 2002-06-21 Vince King  
--  - Modified sp so that the Palletizer PL_Id list (@ProdLinePalList) can be passed   
--    to the report.  This parameter is optional and not required.  @ProdLinePalList can be  
--    used for production lines that require the Uptime to be calculated for downtime  
--    events within a Production Line, rather than events for a specific production unit.  
--    It is not limited to Palletizers, but the concept works for the palletizer whse config.  
--  - Changed 'Fault Desc' to 'Fault' in source data result set.  
--  - Modified so that the PRID parameters, listed above 2002-06-19, are not required.  Added  
--    code to this sp and modified the template.   
--  
-- 2002-06-27 Vince King  
--  - The sp was originally written to flag unscheduled stops as stops with Category <> Scheduled.  
--    In reality, an unscheduled stop is when Category = Unscheduled.  Changed the parameter  
--    @SchedScheduledId to @SchedUnscheduledId and made select inclusive (ScheduledId = @SchedUnscheduledId)  
--  - Parameters: Remove intRptSchedScheduledId and add intRptSchedUnScheduledId.  
--  
-- 2002-07-15 Vince King  
--  - Modified to pickup EquipGroup from the Extended_Info field on all production units.  This allows  
--    the flexibility to group prod units (master and slave) for areas of ownership.  The EquipGroup field  
--    is reported with each event record.  
--  
-- 2002-07-16 Vince King  
--  - Modified the definitions for a Minor Stop, Breakdown, and Process Failure.  The original code by  
--  - MSI did not have the definitions correct.  
--  - Parameters: No changes.  
--  
-- 2002-08-16 Vince King  
--  - I was only selecting Unscheduled primary events in my Stops totals.  It should be ALL primary events.  
--------------------------------------------------------------------------------------------------------------------------------------------------------------  
  
CREATE  PROCEDURE dbo.spLocal_RptStops  
 @StartTime   DateTime,  -- Beginning period for the data.  
 @EndTime   DateTime,  -- Ending period for the data.  
 @BySummary   Int,   -- 0 means a single resultset with all details.  1 means multiple resultsets without details.  
 ...
 @ProdLinePalList  nVarChar(4000)  -- Collection of Prod_Lines.PL_Id for palletizers (or other prod lines that require Uptime  
        -- to be calculated between events for the line instead of between events for prod units).   
AS  
...
CREATE TABLE #ProdUnits (  
 ...
 ScheduleUnit   Int,  
 LineStatusUnit   Int,  
 EquipGroup   nVarChar(100),  
 PRIDVarId   Int,  
 UWSVarId   Int )  
CREATE  TABLE #UWS (  
 MasterPUId   Int,  
 UWSPUId    Int )  
CREATE TABLE  #ProdUnitsEG (  
 PLId    Integer,  
 Master_PUId   Integer,  
 Source_PUId   Integer,  
 Source_PUDesc   nVarChar(100),  
 ExtendedInfo   nVarChar(255),  
 EquipGroup   nVarChar(100) )  
-------------------------------------------------------------------------------  
...
INSERT INTO #ProdUnitsEG (PLId, Master_PUId, Source_PUId, Source_PUDesc, ExtendedInfo)  
 SELECT ppu.PLId, Master_Unit, PU_Id, PU_Desc, Extended_Info  
  FROM #ProdUnits ppu  
   JOIN Prod_Units pu ON ppu.PUId = pu.PU_Id  
  
-- Insert Slave Production Units into #ProdUnitsEG  
INSERT INTO #ProdUnitsEG (PLId, Master_PUId, Source_PUId, Source_PUDesc, ExtendedInfo)  
 SELECT ppu.PLId, Master_Unit, PU_Id, PU_Desc, Extended_Info  
  FROM #ProdUnits ppu  
   JOIN Prod_Units pu ON ppu.PUId = pu.Master_Unit  
...
UPDATE tp  
 SET StopsProcessFailures = CASE WHEN tpu.DelayType <> @DelayTypeRateLossStr  
       ...
       AND (tp.ScheduleId = @SchedUnScheduledId OR tp.ScheduleId IS NULL)  
       AND (tp.CategoryId = @CatProcFailId)  
       AND (tp.StartTime >= @StartTime)  
       THEN 1  
      ELSE 0  
      END  
...
SELECT @strSQL =   
  ...
  BEGIN  
   ...
   SELECT @LoopCtr = @LoopCtr + 1  
  END  
  SELECT @strSQL = @strSQL + ' pueg.EquipGroup [Equipment Group], Comment   
   FROM  #Delays td  
   JOIN  #ProdUnits tpu ON td.PUId = tpu.PUId    JOIN  #ProdUnitsEG pueg ON td.LocationId = pueg.Source_PUId  
...
ReturnResultSets:  
 ...
 BEGIN  
  ...
  -----------------------------------------------------------------------------  
  -- At this point in time, we are only returning the same resultset for Summary  
  -- as is done for Detail.  This section needs to be replaced.  
  -----------------------------------------------------------------------------   
  EXEC(@strSQL)  
  -----------------------------------------------------------------------------  
  ...
  SELECT pl.PL_Desc [Production Line],  
   td.Crew [Team],  
   Sum(Convert(Int, '1')) [Total Causes],  
   Sum(td.DownTime/60.0) [Total DownTime],  
   Sum(td.ReportDownTime/60.0) [Total Event DownTime],  
   Sum(td.UpTime/60.0) [Total UpTime],  
   Sum(td.ReportUpTime/60.0) [Total Event UpTime],  
   Sum(Coalesce(td.Stops, 0)) [Total Stops],  
   Sum(Coalesce(td.UpTime2m, 0)) [Total UpTime < 2 Min],  
   Sum(Coalesce(td.StopsMinor, 0)) [Minor Stops],  
   Sum(Coalesce(td.StopsBreakDowns, 0)) [Break Downs],  
   Sum(Coalesce(td.StopsProcessFailures, 0)) [Process Failures],  
   Sum(Coalesce(td.StopsBlockedStarved, 0)) [Total Blocked Starved]  
   FROM  #Delays td  
   JOIN  Prod_Units pu ON td.PUId = pu.PU_Id  
   JOIN  Prod_Lines pl ON pu.PL_Id = pl.PL_Id  
   LEFT  JOIN  Timed_Event_Fault tef on (td.TEFaultID = TEF.TEFault_ID)  
   GROUP BY pl.PL_Desc, td.Crew  
   ORDER BY pl.PL_Desc, td.Crew  
  -----------------------------------------------------------------------------  
  ...
  SELECT pl.PL_Desc [Production Line],  
   ...
   JOIN  Prod_Units pu ON td.PUId = pu.PU_Id  
   JOIN  Prod_Lines pl ON pu.PL_Id = pl.PL_Id  
   GROUP BY pl.PL_Desc, td.Shift  
   ORDER BY pl.PL_Desc, td.Shift  
  -----------------------------------------------------------------------------  
  ...
  SELECT pl.PL_Desc [Production Line],  
   ...
   JOIN  Prod_Lines pl ON pu.PL_Id = pl.PL_Id  
   JOIN  Products p ON td.ProdId = p.Prod_Id  
   GROUP BY pl.PL_Desc, p.Prod_Desc  
   ORDER BY pl.PL_Desc, p.Prod_Desc  
  -----------------------------------------------------------------------------  
  ...
  SELECT pl.PL_Desc [Production Line],  
   ...
   JOIN  #ProdUnits tpu ON td.PUId = tpu.PUId  
   JOIN  Prod_Lines pl ON pu.PL_Id = pl.PL_Id  
   GROUP BY pl.PL_Desc, tpu.DelayType  
   ORDER BY pl.PL_Desc, tpu.DelayType  
  -----------------------------------------------------------------------------  
  ...
  SELECT pl.PL_Desc [Production Line],  
   ...
   JOIN  Prod_Lines pl ON pu.PL_Id = pl.PL_Id  
   LEFT JOIN Event_Reason_Catagories erc2 ON td.CategoryId = erc2.ERC_Id  
   GROUP BY pl.PL_Desc, erc2.ERC_Desc  
   ORDER BY pl.PL_Desc, erc2.ERC_Desc  
  ...
  SELECT pl.PL_Desc [Production Line],  
   ...
   JOIN  Prod_Lines pl ON pu.PL_Id = pl.PL_Id  
   LEFT JOIN Event_Reason_Catagories erc1 ON td.ScheduleId = erc1.ERC_Id  
   GROUP BY pl.PL_Desc, erc1.ERC_Desc  
   ORDER BY pl.PL_Desc, erc1.ERC_Desc  
 END  
  
 DROP TABLE #ErrorMessages  
 DROP TABLE #ProdUnits  
 DROP TABLE #ProdUnitsEG  
 DROP TABLE #Delays  
 DROP TABLE #PRIDs  
 DROP TABLE #UWS  
