...
/*
--------------------------------------------------------------
Name: spLocal_STLS_parmdelupd_UpdatePatternAll1
Purpose: Update  a pattern.
--------------------------------------------------------------
Build #8
...
CREATE  PROCEDURE spLocal_STLS_parmdelupd_UpdatePatternAll1
...
SET @charTeamDelimiter = ','
	...
	-- If pattern name already exists, exit
	SELECT * 
	FROM Local_PG_Patterns
	WHERE PatternName = @PatternName
	AND PatternID <> @PatternIDNumber			-- vinayak added
	...
	--0.  Delete Existing Pattern (Replace with new)

	DELETE FROM Local_PG_Pattern_Schedule			-- vinayak moved
	WHERE PatternID = @PatternIDNumber			-- vinayak moved

	DELETE FROM Local_PG_Patterns				-- vinayak moved
	WHERE PatternID = @PatternIDNumber			-- vinayak moved
	...
	BEGIN
		...
		--4.  Get 1 Record from @PatternShiftTimes
