CREATE PROCEDURE dbo.spMSICalc_PercentWaste
...
if (@ActualQualityLoss + @ActualProduction) > 0
  Begin
    Select @PercentWaste = (@ActualQualityLoss / (@ActualQualityLoss + @ActualProduction)) * 100
  End
if @PercentWaste is not NULL  Select @Output = Convert(varchar(25), @PercentWaste)
