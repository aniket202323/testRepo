CREATE   PROCEDURE [dbo].[spRS_AdminGetMillStartTime] 
AS
-------------------------------------
-- LOCAL VARIABLES
-------------------------------------
...
WHILE 	 DatePart(WeekDay, @WeekEndDate) <> 7
 	 BEGIN
 	  	 ...
 	  	 CONTINUE
 	 END
select @t 'Mill_Start_Time', Convert(DateTime, @WeekEndDate, 120) 'WeekEndDate', Convert(DateTime, @WeekStartDate,120) 'WeekStartDate', Convert(DateTime, @AccountingDate, 120) 'AccountingDate', @StartOfDaySeconds 'StartOfDaySeconds', @ShiftInterval 'ShiftInterval'
