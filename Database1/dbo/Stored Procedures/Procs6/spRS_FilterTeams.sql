CREATE PROCEDURE [dbo].spRS_FilterTeams
@PuId 	  	  	  	 int ,
@StartTime 	  	  	 datetime,
@EndTime 	    	  	 datetime,
@Mask   	  	  	  	 VARCHAR (100),
...
If @ExcludeStr Is Not Null
  ...
  End  
 	 ...
 	 /*SELECT @SQLStatement =  	 ' SELECT DISTINCT CREW_DESC AS TEAM_DESC ' +
 	  	  	  	  	  	  	  	  	  	 ...
 	  	  	  	  	  	  	  	  	  	 ' ORDER  BY CREW_DESC' 	  	 */
 	 INSERT INTO #temp (team_desc) SELECT DISTINCT CREW_DESC AS TEAM_DESC 
 	 FROM DBO.CREW_SCHEDULE C
 	 WHERE C.PU_ID = @PuID AND C.CREW_DESC LIKE @FullMask
