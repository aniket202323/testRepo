CREATE   PROCEDURE [dbo].[spRS_GetTimeOptions] 
@Option_Id int = Null,
...
SELECT @MillStartDate = dateadd(dd,0, datediff(dd,0,dbo.fnServer_CmnConvertTime(dbo.fnServer_CmnGetDate(GETUTCDATE()),@DBTimeZone,@InTimeZone)))
...
-- Get temporary dates.
-------------------------------------------------------------------------------
SELECT  	  @AccountingDate = Convert(DateTime, Floor(Convert(Float, DateAdd(Second, -Convert(Int, @StartOfDaySeconds), dbo.fnServer_CmnConvertTime(dbo.fnServer_CmnGetDate(GETUTCDATE()),@DBTimeZone,@InTimeZone)))))
-- Week Start date.
...
Fetch Next From Cursor1 Into @MyId, @Date_Type_Id
  If (@@Fetch_Status = 0)
    Begin
  	    	  ...
  	    	  SELECT @SQLString = REPLACE(@SQLString,'getdate()','dbo.fnServer_CmnConvertTime(dbo.fnServer_CmnGetDate(GETUTCDATE()),'''+ @DBTimeZone + ''',' + ''''+ @InTimezone + ''')')
  	    	    	  	 --END
    ...
    Begin -- Begin Loop Here
      Select @s = Start_Date_SQL, @e = End_Date_SQL from Report_Relative_Dates where RRD_id = @MyId
        ...
        Begin
  	    	    	  ...
  	    	    	  --------------------------
  	    	    	  SELECT @s = REPLACE(@s,'getdate()','dbo.fnServer_CmnConvertTime(dbo.fnServer_CmnGetDate(GETUTCDATE()),'''+ @DBTimeZone + ''',' + ''''+ @InTimezone + ''')')
  	    	    	  SELECT @e = REPLACE(@e,'getdate()','dbo.fnServer_CmnConvertTime(dbo.fnServer_CmnGetDate(GETUTCDATE()),'''+ @DBTimeZone + ''',' + ''''+ @InTimezone + ''')')
...
--update #t set Start_Time= case when (ISDATE(Start_Time)=1)
-- 	  	  	  	  	  	  	  	  	 then
-- 	  	  	  	  	  	  	  	  	  	 Convert(Varchar(30),[dbo].[fnServer_CmnConvertFromDbTime] (convert(datetime,Start_Time),@InTimeZone),120)
-- 	  	  	  	  	  	  	  	  	 else
-- 	  	  	  	  	  	  	  	  	  	 Start_Time
-- 	  	  	  	  	  	  	  	  	 end,
-- 	  	  	  	 End_Time = case when (ISDATE(End_Time)=1)
-- 	  	  	  	  	  	  	  	  	 then
-- 	  	  	  	  	  	  	  	  	  	 Convert(Varchar(30),[dbo].[fnServer_CmnConvertFromDbTime] (convert(datetime,End_Time),@InTimeZone),120)
-- 	  	  	  	  	  	  	  	  	 else
-- 	  	  	  	  	  	  	  	  	  	 End_Time
-- 	  	  	  	  	  	  	  	  	 end 
If @Option_Id is null
  Select Option_Id, Date_Type_Id, Description, Start_Time, End_Time 
  From #t
