...
---------------------------------------------------------------------------------------------------
CREATE      	PROCEDURE [dbo].[spS88_RSBatch6Reader_XQGSS1]
...
BEGIN
	...
	*/
	...
	SELECT @SQL = '(Select distinct Recipe,BatchID,Descript From ' + @tablename+ 
    ' where Batchid = '+''''+@BatchName+''''  +'AND '+'(Descript  like ' +''''+@lotid+'''' +') AND Pvalue <>'+Char(39)+' '+Char(39)+')'



		INSERT @Recipe (Recipe,Batchid,Descript)
	EXEC (@Sql)
...
SET @Max = (SELECT COUNT(*) FROM @Recipe) + 1
 ...
 SET @Descript = (SELECT Descript FROM @Recipe WHERE ID = (@Min - 1))
...
ELSE
BEGIN
SET @recipe1 = ( SELECT Recipe FROM @Recipe WHERE ID = (@Min-1))
END
...
EXEC ( @sql)
	...
	Select * From #temp order by recordno 
	
	

	 DELETE FROM #Temp
	 WHERE DESCRIPT LIKE @lotid AND PValue LIKE ' '

	 	 DELETE FROM #Temp
	 WHERE DESCRIPT LIKE @NetWeight AND PValue LIKE '0'


	 Select @MinLOTid = SUBSTRING(@Descript,LEN(@descript) , Len(@descript))



	 UPDATE #Temp
	SET Multilot = 0
	WHERE (Descript like 'LOT_ID' Or Descript LIKE 'NET_Weight')
	AND Recipe LIKE '%'+@recipe2+'%'



	DELETE FROM #TEMP WHERE Multilot = 0
	  ...
	  Update #Temp 
	  SET DEscript = 'LOT_ID'
	  WHERE Descript like 'LOT_ID_'+Convert(Varchar,@MinLOTid)

	   Update #Temp 
	   SET DEscript = 'Net_Weight'
	   WHERE Descript like 'Net_Weight_'++Convert(Varchar,@MinLOTid)



	   	 SET @RunningRecordno = (SELECT RecordNo FROM #Temp 
							WHERE Descript like '%State%Change%' and Pvalue like '%Running%' AND Recipe like '%'+@recipe2+'%')
...
END
 ...
 SELECT @Sql= '(SELECT LCLTime, Area, ProcCell, Unit, BatchID, UniqueID, 
						...
						+ ' AND LCLTime <= DATEADD(DAY, 3, GETDATE()))  ORDER BY Recordno'
