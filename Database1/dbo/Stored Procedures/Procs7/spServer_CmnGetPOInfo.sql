CREATE PROCEDURE dbo.spServer_CmnGetPOInfo
@PPStartId int,
@TimeStamp DateTime,
@InPUId int,
@OutPUId int OUTPUT,
    ...
    If (@EndTime < @Timestamp)
      Begin
        ...
        If (@Status is NULL) Or (@Status <> 3)
          return
      End
  select  --CASE WHEN LEAD(Start_time,1) OVER (Order by PP_START_ID) <> Start_time THEN LEAD(Start_time,1) OVER (Order by PP_START_ID) ELSE START_Time end 
 	 TOP 1
 	 Start_Time, 
           EndTime = Case 
             When End_Time Is NULL Then dbo.fnServer_CmnGetDate(GetUTCDate())
             Else dbo.fnServer_CmnGetDate(GetUTCDate())--End_Time
           End
      from Production_Plan_Starts where 
 	   --PP_Id = @minPPID  and 
