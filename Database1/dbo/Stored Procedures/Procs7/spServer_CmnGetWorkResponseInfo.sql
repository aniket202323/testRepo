CREATE PROCEDURE [dbo].[spServer_CmnGetWorkResponseInfo]
@WRId int,
@Found int OUTPUT,
...
Select @S95Guid = NULL
Select @S95Guid = S95_Guid From S95_Event Where (Event_Id = @WRId) And (Event_Type = 32)
If (@S95Guid Is Null)
  Begin
    Select @ErrorMsg = 'S95_Event Not Found for Event_Id (' + Convert(nVarChar(20),@WRId) + ')'
    return
  End
 Select @EquipmentId = EquipmentId,
 	 ...
 	 From WorkResponse
 	 Where (WorkResponseId = @S95Guid)
If (@EquipmentId Is Null)
  Begin
    Select @ErrorMsg = 'WorkResponse Not Found for S95 Event_Id (' + Convert(nVarChar(20),@WRId) + ') And S95_Guid (' + Convert(nVarChar(100),@S95Guid) +')'
    return
  End
If (@EndTime Is Null)
  Begin
    Select @ErrorMsg = 'SegmentResponse for S95 Event_Id (' + Convert(nVarChar(20),@WRId) + ') Missing EndTime)'
    return
  End
...
insert into @Results (MaterialDefinitionId)
 	 exec spServer_CmnGetWorkRespMaterials @S95Guid
update @Results
