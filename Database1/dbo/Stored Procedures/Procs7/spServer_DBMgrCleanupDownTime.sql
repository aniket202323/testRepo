Create Procedure dbo.spServer_DBMgrCleanupDownTime
@DowntimeId int,
@NewTime Datetime,
...
If (@MasterPUId Is NULL)
  Return
If (@NewTime Is NULL)
  Execute spServer_DBMgrDeleteEventData @MasterPUId,@TimeStamp, 2,@ReturnResultSet, null, @UserId
Else 
  Begin
    Execute spServer_DBMgrMoveEventData @MasterPUId,@TimeStamp,@NewTime,2,@ReturnResultSet, null, null, 0, @UserId
  End
Declare Downtime_Sheet_Cursor INSENSITIVE CURSOR
  For (Select Sheet_Id From Sheets Where (Event_Type = 0) And (Sheet_Type = 20) And (Master_Unit = @MasterPUId))
