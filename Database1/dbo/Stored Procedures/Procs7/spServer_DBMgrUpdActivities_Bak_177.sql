CREATE PROCEDURE dbo.[spServer_DBMgrUpdActivities_Bak_177] 
 	 @ActivityId 	  	    Int,
...
BEGIN
  IF @DebugFlag = 1 INSERT INTO Message_Log_Detail (Message_Log_Id, Message) Values(@ID, 'Bad TransType: Return(-100)' )
 	 Return(-100)
END 
IF @ActivityTypeId  Not IN (1,2,3,4,5) or @ActivityTypeId Is Null
BEGIN
...
BEGIN
    ...
    END
 	 ...
 	 BEGIN
 	  	 ...
 	  	 END
 	 END
  	  ELSE IF @ActivityTypeId IN ( 2,3,4,5) --Production Event
 	 BEGIN
 	  	 IF @ActivityTypeId = 2
 	  	 BEGIN
 	 ...
 	 BEGIN -- Support updating the Activity Desc and start_time for event based activities
 	  	 ...
 	  	 BEGIN
 	  	  	 ...
 	  	  	 BEGIN
 	  	  	  	 ...
 	  	  	  	 Return(-200)
 	  	  	 END
 	  	  	 SELECT @EventDesc = Event_Num, @KeyIdDateTime = TimeStamp
 	  	  	  	 FROM [Events] e
 	  	  	  	 WHERE  e.Event_Id = @KeyIdInt
