CREATE PROCEDURE dbo.[spServer_DBMgrUpdColumn2_Bak_177]
  @Sheet_Id int,
  @Is_Delete int,
  @Result_On datetime,
...
SELECT @MYId = Sheet_Id,@Sheet_Type_Id = Sheet_Type,@pu_Id= case When @Sheet_Type_Id in (21,23) then Master_Unit Else NULL end FROM Sheets WHERE Sheet_Id = @Sheet_Id
SET @Activity_Type_Id = CASE WHEN @Sheet_Type_Id =23 THEN 4 ELSE CASE WHEN @Sheet_Type_Id =21 THEN 5 ELSE 1 END END 
IF @MYId IS NULL
BEGIN
