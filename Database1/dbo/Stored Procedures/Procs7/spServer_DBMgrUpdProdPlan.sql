CREATE PROCEDURE dbo.spServer_DBMgrUpdProdPlan
@PPId int OUTPUT,
@TransType int,
@TransNum int,
@PathId int, 
...
Declare @DebugFlag tinyint,
 	   ...
 	   @MyOwnTrans 	  	  	 Int
 	   Declare @Implied_Sequence Int,@Implied_Sequence_Offset Int, @Implied_Sequence_String nVarChar(15),@CurrentForecaststartDate Datetime,@Sequence_Offset int
If @@Trancount = 0
 	 SELECT @MyOwnTrans = 1
Else
 	 SELECT @MyOwnTrans = 0
...
If @TransType = 1 or (@TransType = 2 and @TransNum = 96) or (@TransType = 2 and (@TransNum = 0 or @TransNum = 2) and (@Start_Date <> @ForecastStartTime or @End_Date <> @ForecastEndTime))
    ...
    If Not (@TransNum = 93 or @TransNum = 94)
      Begin
        ...
        --    End
 	  	 IF @PPID IS NOT NULL
 	  	 BEGIN
 	  	  	 SELECT @ImpliedSequence = Implied_Sequence,@Sequence_Offset = Implied_Sequence_Offset,@CurrentForecaststartDate = Forecast_Start_Date,@PathId = case when @Transnum = 96 then @PathId Else Path_Id END FROM Production_Plan WHERE PP_Id = @PPId 	 
 	  	  	  IF @TransNum = 96 and @PathId IS NOT NULL
 	  	  	  BEGIN
 	  	  	  	 SELECT @ForecastStartTime = ISNULL(@ForecastStartTime,@CurrentForecaststartDate)
 	  	  	  END
 	  	  	 If Datediff(s,@CurrentForecaststartDate,@ForecastStartTime ) <> 0 OR (@Transnum = 96 AND @PathId IS NOT NULL)
 	  	  	 Begin
 	   
 	  	  	  	 SET @Implied_Sequence_String = dbo.fnCmn_getuniqueIntForDate(@ForecastStartTime,@PathId)
 	  	  	  	 SET @Sequence = CAST(LEFT(@Implied_Sequence_String,PATINDEX('%;%',@Implied_Sequence_String)-1) as Int)
 	  	  	  	 SET @Sequence_Offset = 0
 	  	  	  	 SET @Sequence_Offset = Cast( Replace(@Implied_Sequence_String,LEFT(@Implied_Sequence_String,PATINDEX('%;%',@Implied_Sequence_String)-1)+';','')     As int)
 	  	  	 End
 	  	  	 ELSE
 	  	  	 BEGIN
 	  	  	  	 SET @Sequence = @ImpliedSequence
 	  	  	  	 SET @Sequence_Offset = @Sequence_Offset
 	  	  	  	  IF @TransNum = 96 and @PathId IS NULL
 	  	  	  	  BEGIN
 	  	  	  	  	 SET @Sequence = NULL
 	  	  	  	  	 SET @Sequence_Offset = NULL
 	  	  	  	  END
 	  	  	 END
 	  	 END
      End
  ...
  Begin /**********Update************/
    ...
    if @ImpliedSequence is NULL
      Begin
        SELECT @ImpliedSequence = Implied_Sequence,@Sequence_Offset = Implied_Sequence_Offset,@CurrentForecaststartDate = Forecast_Start_Date,@PathId =Path_Id FROM Production_Plan WHERE PP_Id = @PPId
      End
 	  	 If Datediff(s,@CurrentForecaststartDate,@ForecastStartTime ) <> 0
 	  	 Begin
      ...
      Begin
        If @ProcessOrder is NULL or LTrim(RTrim(@ProcessOrder)) = ''
          ...
          End
           ...
           Cast( Replace(@Implied_Sequence_String,LEFT(@Implied_Sequence_String,PATINDEX('%;%',@Implied_Sequence_String)-1)+';','')     As int)
 	  	    IF @PathId IS NULL
 	  	  	 SELECT @Implied_Sequence = NULL, @Implied_Sequence_Offset = NULL
 	  	   /*
      ...
      Begin  
       ...
       SELECT @PPId = NULL, @PPStatusId = 1, @ProdId = Prod_Id, @BlockNumber = Block_Number, @CommentId = NULL, 
          ...
          WHERE PP_Id = @ParentPPId
 	  	   ...
 	  	   SET @Implied_Sequence = CAST(LEFT(@Implied_Sequence_String,PATINDEX('%;%',@Implied_Sequence_String)-1) as Int)
 	  	   SET @Implied_Sequence_Offset = 0
 	  	   SET @Implied_Sequence_Offset = Cast( Replace(@Implied_Sequence_String,LEFT(@Implied_Sequence_String,PATINDEX('%;%',@Implied_Sequence_String)-1)+';','')     As int)
        Insert Into Production_Plan (Path_Id, Process_Order, PP_Status_Id, Prod_Id, Block_Number, Forecast_Start_Date, 
   	  	  	 Forecast_End_Date, Forecast_Quantity, Implied_Sequence, Comment_Id, PP_Type_Id, Production_Rate, Source_PP_Id, 
 	    	  	 User_Id, Entry_On, Adjusted_Quantity, Parent_PP_Id, Control_Type, BOM_Formulation_Id,User_General_1,User_General_2,User_General_3,Extended_Info,Implied_Sequence_Offset)
        Values (@PathId, @ProcessOrder, @PPStatusId, @ProdId, @BlockNumber, @ForecastStartTime, @ForecastEndTime, 
...
Else If @TransType = 2
  Begin /**********Update************/
    ...
    Else If @TransNum = 96
      Begin
 	   Declare @PathBeforeUnbinding Int
 	   SELECT @PathBeforeUnbinding = Case when @PathId IS NULL Then Path_Id End From Production_Plan where pp_Id = @PPId
        Update Production_Plan Set Path_Id = @PathId, User_Id = @UserId, Entry_On = @EntryOn, Implied_Sequence = @Sequence,Implied_Sequence_Offset = @Sequence_Offset
          WHERE PP_Id = @PPId
 	  	  
  	    	    IF @PathBeforeUnbinding IS NOT NULL
  	    	    BEGIN  	   
  	    	        IF EXISTS(SELECT 1 From Production_Plan where PAth_id = @PathBeforeUnbinding Group by Implied_Sequence having count(0) >1)
 	  	  	    Begin
  	    	       	  ;WITH S1 as (Select Implied_Sequence From Production_Plan where PAth_id = @PathBeforeUnbinding Group by Implied_Sequence Having count(0) >1)
 	  	  	  	  ,S AS (Select PP_ID,P.Implied_Sequence,Implied_Sequence_Offset, Row_number() Over (Partition By Path_Id, P.Implied_Sequence Order by Implied_Sequence_Offset)-1 ActualImpliedSequenceOffset From Production_Plan P JOIN S1 ON S1.Implied_Sequence = P.Implied_Sequence where PAth_id = @PathBeforeUnbinding)
   	     	    	  UPDATE P SET P.Implied_Sequence_Offset = S.ActualImpliedSequenceOffset
   	     	    	  From Production_Plan P Join S On S.PP_Id =P.PP_Id and S.Implied_Sequence = P.Implied_Sequence
  	   END
 	  END
        if @@ERROR <> 0  
          Begin
   	 ...
   	 Else If @TransNum = 0
     	 ...
     	 SELECT @PathId = Coalesce(@PathId, Path_Id), 
     	  	  	 ...
     	  	  	 @Sequence = Coalesce(@Sequence, Implied_Sequence), 
 	  	  	  	  @Sequence_Offset = Coalesce(@Sequence_Offset,Implied_Sequence_Offset),
    ...
    --Fields Not Editable In Client
 	 DECLARE @OldImpliedSequence int,@OldImpliedSequenceOffset int
  	    	  SELECT @AdjustedQuantity = Coalesce(@AdjustedQuantity, Adjusted_Quantity), 
      @ParentPPId = Coalesce(@ParentPPId, Parent_PP_Id),@OldImpliedSequence = Implied_Sequence,@OldImpliedSequenceOffset=Implied_Sequence_Offset
  	    	   FROM Production_Plan
  	    	   WHERE (PP_Id = @PPId)
    Update Production_Plan 
 	  	 ...
 	  	 Implied_Sequence = @Sequence,
 	  	  Implied_Sequence_Offset = @Sequence_Offset,
 	  	 Entry_On = @EntryOn,
    ...
    WHERE PP_Id = @PPId
 	 /*
 	 cHECK IF THE FORECASTSTARTDATE EQUIVALENT EPOCH DATE IS EQUAL TO THAT OF IMPLIED SEQUENCE OR NOT
 	  	 IF YES THEN DO NOTHING
 	  	 ELSE
 	  	  	 REARRANGE THE ORDERS WITH IMPLIED SEQUENCE THAT THE CURRENT ORDER HAD PREVIOUSLY BEFORE UPDATE
 	 */ 
  	  IF (@TransNum in (0, 2) and @TransType = 2) OR (@TransNum in(98,99))
 	 BEGIN
 	  
  	    	   IF EXISTS(SELECT 1 From Production_Plan where PAth_id = @PathId Group by Implied_Sequence having count(0) >1)
 	  	   BEGIN
  	  ;WITH
 	  S1 as (Select Implied_Sequence From Production_Plan where PAth_id = 2 Group by Implied_Sequence Having count(0) >1)
 	  ,S AS (Select PP_ID,P.Implied_Sequence,Implied_Sequence_Offset, Row_number() Over (Partition By Path_Id, P.Implied_Sequence Order by Implied_Sequence_Offset)-1 ActualImpliedSequenceOffset From Production_Plan P join S1 on S1.Implied_Sequence = P.Implied_Sequence where PAth_id = @PathId)
  	  UPDATE P SET P.Implied_Sequence_Offset = S.ActualImpliedSequenceOffset
  	  From Production_Plan P Join S On S.PP_Id =P.PP_Id and S.Implied_Sequence = P.Implied_Sequence
  	    	   END
    ...
    if @@ERROR <> 0  
      Begin
        ...
        If @DebugFlag = 1 Insert into Message_Log_Detail (Message_Log_Id, Message) Values(@ID, 'END')
       	 Return (-100)
      End
    Declare @DeletedImpliedSequence int
 	 SELECT @DeletedImpliedSequence = Implied_Sequence from Production_Plan WHERE PP_Id = @PPId
    Delete FROM Production_Plan WHERE PP_Id = @PPId
 	  IF EXISTS(SELECT 1 From Production_Plan where PAth_id = @PathId and Implied_Sequence = @DeletedImpliedSequence Group by Implied_Sequence having count(0) >1)
 	  Begin
  	  ;WITH S AS (Select PP_ID,Implied_Sequence,Implied_Sequence_Offset, Row_number() Over (Partition By Path_Id, Implied_Sequence Order by Implied_Sequence_Offset)-1 ActualImpliedSequenceOffset From Production_Plan where PAth_id = @PathId)
  	  UPDATE P SET P.Implied_Sequence_Offset = S.ActualImpliedSequenceOffset
  	  From Production_Plan P Join S On S.PP_Id =P.PP_Id and S.Implied_Sequence = P.Implied_Sequence
  	  WHERE P.Implied_Sequence =@DeletedImpliedSequence
    End
    if @@ERROR <> 0  
      Begin
        -- Need to add messages before using: RAISERROR(50003, 11, -1, 'Events')
