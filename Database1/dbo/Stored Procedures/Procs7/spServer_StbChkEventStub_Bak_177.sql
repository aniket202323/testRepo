CREATE PROCEDURE dbo.[spServer_StbChkEventStub_Bak_177]
@PU_Id int,
@TimeStamp datetime,
@SheetId int,
@DoAll int OUTPUT,
...
if (@SheetId > 0)
  begin
    ...
    Select @Prev_TimeStamp = Max(Result_On) From Sheet_Columns Where (Sheet_Id = @SheetId) And (Result_On < @TimeStamp)
  ...
  begin
    ...
    if (@EventType = 1) -- Production
      Select @Prev_TimeStamp = Max(Timestamp) From Events Where (PU_Id = @PU_Id) And (TimeStamp < @TimeStamp)
    else if (@EventType = 2) -- Downtime
      Select @Prev_TimeStamp = Max(End_Time) From Timed_Event_Details Where (PU_Id = @PU_Id) And (End_Time < @TimeStamp)
    else if (@EventType = 3) -- Waste
      Select @Prev_TimeStamp = Max(Timestamp) From Waste_Event_Details Where (PU_Id = @PU_Id) And (TimeStamp < @TimeStamp)
    else if (@EventType = 14) -- UDE
      Select @Prev_TimeStamp = Max(End_Time) 
        From User_Defined_Events With(Index(UDE_IDX_PUIdESIdEndTime))
        Where (PU_Id = @PU_Id) And (End_Time < @TimeStamp) And (Event_Subtype_id = @EventSubtype)
    else if (@EventType = 22) -- Uptime
      Select @Prev_TimeStamp = Max(Start_Time) From Timed_Event_Details Where (PU_Id = @PU_Id) And (Start_Time < @TimeStamp)
    If (@Prev_TimeStamp Is NULL)
      return
    ...
    if (@EventType = 1) -- Production
      Select @LastResetTime = Max(TimeStamp) From Events Where (PU_Id = @PU_Id) And (TimeStamp < @TimeStamp) And (TimeStamp > @Prev_Run_Start) and (Testing_Status = 3)
    else if (@EventType = 14) -- UDE
      Select @LastResetTime = Max(End_Time) From User_Defined_Events With(Index(UDE_IDX_PUIdESIdEndTime)) Where (PU_Id = @PU_Id) And (End_Time < @TimeStamp) And (End_Time > @Prev_Run_Start) And (Event_Subtype_id = @EventSubtype) and (Testing_Status = 3)
