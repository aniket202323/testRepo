Create Procedure dbo.spWDGetWasteDetailsTest
@pPU_Id int,
@pStartTime datetime,
@pEndTime datetime
AS
...
select Event_Id, Event_Num, Timestamp, Event_Status, Applied_Product
  ...
  where pu_id = @pPu_Id and
        ...
        event_status > 2
/* Event Based */
IF EXISTS (SELECT 1 FROM WASTE_EVENT_DETAILS WHERE PU_Id = @pPu_Id)
BEGIN
...
from 
@AllEvents e
LEFT OUTER JOIN waste_event_details d WITH(INDEX(WEvent_Details_IDX_EventId)) on e.event_id = d.event_id and d.event_Id IS NOT NULL
END
/* Time Based */ 
INSERT INTO @WasteRecords(Event_Id,
 	  	  	  	  	  	  	 Event_Num,
