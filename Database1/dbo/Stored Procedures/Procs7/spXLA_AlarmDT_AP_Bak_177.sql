...
--
CREATE PROCEDURE dbo.[spXLA_AlarmDT_AP_Bak_177]
 	   @Var_Id 	  	 Int
 	 , @Var_Desc 	  	 Varchar(50)
 	 , @Start_Time 	  	 DateTime
 	 , @End_Time 	  	 DateTime
...
RETRIEVE_ORIGINAL_PRODUCT_RESULT_SET:
  If @TimeSort = 1 --Ascending Order
    BEGIN
        SELECT t.Alarm_Id
             , t.Alarm_Desc
             , [Start_Time] = Case When t.Crew_Start Is NULL Then dbo.fnServer_CmnConvertFromDbTime(t.Start_Time,@InTimeZone)
                                   When t.Crew_Start >= t.Start_Time Then dbo.fnServer_CmnConvertFromDbTime(t.Crew_Start,@InTimeZone) Else dbo.fnServer_CmnConvertFromDbTime(t.Start_Time,@InTimeZone) End
             , [End_Time]   = Case When t.Crew_End Is NULL Then dbo.fnServer_CmnConvertFromDbTime(t.End_Time,@InTimeZone)
                                   When t.Crew_End <= t.End_Time Then dbo.fnServer_CmnConvertFromDbTime(t.Crew_End,@InTimeZone) Else dbo.fnServer_CmnConvertFromDbTime(t.End_Time,@InTimeZone) End
             , t.Duration
