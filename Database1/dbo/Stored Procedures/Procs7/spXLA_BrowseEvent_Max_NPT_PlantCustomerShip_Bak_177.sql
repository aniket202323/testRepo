/* NOTE 
   Added crew,shift filter: Defect #24503: mt/10-3-2002
   ...
   (5) spXLA_BrowseEvent_Max_PlantFilter
   (6) spXLA_BrowseEvent_Max_PlantShipFilters
   (7) spXLA_BrowseEvent_Max_PlantCustomerFilters
   (8) spXLA_BrowseEvent_Max_NPT_PlantCustomerShip 	  	  	 'This stored Procedure
*/
-- USAGE: 
-- spXLA_BrowseEvent_Max should be called only when maximum join condition is met and Customer_Code is supplied as 
-- the only filter. mt/10-3-2002
--
CREATE PROCEDURE dbo.[spXLA_BrowseEvent_Max_NPT_PlantCustomerShip_Bak_177] 
...
Else --@MasterUnit NOT NULL
  ...
  END
--EndIf @MasterUnit...
GOTO GET_RESULTSET_FROM_TEMP_EVENTS
...
GOTO GET_RESULTSET_FROM_TEMP_EVENTS
GET_RESULTSET_FROM_TEMP_EVENTS:
  ...
  Else If @MyType = @HasStringHasStatusAsc --3
    BEGIN
        SELECT Primary_Event_Num = e.Event_Num, Event_Id = e.Event_Id, Start_Time = dbo.fnServer_CmnConvertFromDbTime(e.Start_Time,@InTimeZone),TimeStamp = dbo.fnServer_CmnConvertFromDbTime(e.TimeStamp,@InTimeZone)  
             ...
             , sh.Shipment_Number, e.Extended_Info
          FROM #Events e
          JOIN Products p ON p.Prod_Id = e.Prod_Id
          JOIN Prod_Units pu ON pu.Pu_Id = e.Pu_Id
          LEFT OUTER JOIN Event_Details ed ON ed.Event_Id = e.Event_Id
