/* NOTE 
   Added crew,shift filter: Defect #24503: mt/10-2-2002
   ...
   (3) spXLA_BrowseEvent_Shipment_NPT_CustomerFilter 	  	  	 'This stored Procedure: mt/01-22-2003
   ...
   (7) spXLA_BrowseEvent_Shipment_PlantCustomerFilters
   (8) spXLA_BrowseEvent_Shipment_PlantCustomerShip
*/
-- USAGE: 
-- spXLA_BrowseEvent_Shipment_NPT_CustomerFilter should be called when shipment join condition (see above) is met and 
-- Customer_Code filter is specified. MSi/mt/10-2-2002
--
CREATE PROCEDURE dbo.[spXLA_BrowseEvent_Shipment_NPT_CustomerFilter_Bak_177] 
...
If @EventStatusFilter Is NOT NULL
  BEGIN
    ...
    SELECT @CurrentEventStatus = LTRIM(RTRIM(@CurrentEventStatus))
      If @CurrentEventStatus <> '' 
        BEGIN
          SELECT @Event_Status = CONVERT(TinyInt, @CurrentEventStatus)
          SELECT @EventStatusCount = @EventStatusCount + 1
          INSERT #Event_Status VALUES(@Event_Status)
        END
      --EndIf      
