/* NOTE 
   Added Crew,Shift Filters: Defect #24503: mt/10-2-2002
   ECR #24541: mt/1-22-2003: added fields "Conformance" and "Testing_Prct_Complete"
   ...
   NOT to add more branches of SQL Statement into spXLA_BrowseEvent_Shipment but to expand into 8 stored procedures listed 
   below. Future bug fix to core code, if any, should be done in all 8 stored procedures.  
   The 8 stored procedures are:
   (1) spXLA_BrowseEvent_Shipment 	  	  	  	 'Master copy
   (2) spXLA_BrowseEvent_Shipment_ShipFilter
   (3) spXLA_BrowseEvent_Shipment_CustomerFilter
   (4) spXLA_BrowseEvent_Shipment_CustomerShipFilters
   (5) spXLA_BrowseEvent_Shipment_PlantFilter
   (6) spXLA_BrowseEvent_Shipment_PlantShipFilters
   (7) spXLA_BrowseEvent_Shipment_PlantCustomer
   (8) spXLA_BrowseEvent_Shipment_NPT_PlantCustomerShip 	  	 'This stored Procedure; name is under 50-char limit : mt/01-22-2003
*/
-- USAGE: 
-- spXLA_BrowseEvent_Shipment_NPT_PlantCustomerShip should be called when shipment join condition (see above) is met and
-- Plant_Order_Number, Customer_Code, and Shipment_Number filters are specified.  mt/10-2-2002
--
CREATE PROCEDURE dbo.[spXLA_BrowseEvent_Shipment_NPT_PlantCustomerShip_Bak_177] 
...
CREATE TABLE #Events (Event_Num Varchar(25), Event_Id Int, Pu_Id Int, Prod_Id Int, Start_Time DateTime, TimeStamp DateTime
                    ...
                    , Event_Conformance TinyInt NULL, Testing_Prct_Complete TinyInt NULL , NPT tinyint NULL)   -- ECR #24541: mt/1-22-2003
      ...
      BEGIN
        INSERT INTO #Events
          SELECT e.Event_Num, e.Event_Id, e.Pu_Id, ps.Prod_Id, COALESCE(e.Start_Time, DATEADD(ss, -1, e.TimeStamp)), e.TimeStamp, e.Extended_Info, e.Event_Status, e.Applied_Product
               , e.Conformance, e.Testing_Prct_Complete ,NULL                                      --ECR #24541: mt/1-22-2003
            FROM Events e
  ...
  If @MyType = @NoStringHasStatusAsc --1
    BEGIN
        SELECT Primary_Event_Num = e.Event_Num, Event_Id = e.Event_Id, Start_Time = dbo.fnServer_CmnConvertFromDbTime(e.Start_Time,@InTimeZone),TimeStamp = dbo.fnServer_CmnConvertFromDbTime(e.TimeStamp,@InTimeZone)  
             , e.Event_Conformance, e.Testing_Prct_Complete                                  --ECR #24541: mt/1-22-2003
