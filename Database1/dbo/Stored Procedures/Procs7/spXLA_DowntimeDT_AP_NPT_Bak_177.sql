-- DESCRIPTION: spXLA_DowntimeDT_AP_NPT() is modified from spXLA_DowntimeDetail_AP. Changes include addition of 6 input
...
--
CREATE PROCEDURE dbo.[spXLA_DowntimeDT_AP_NPT_Bak_177]
 	   @Start_Time 	  	 DateTime
 	 , @End_Time 	  	 DateTime
...
-- RETURN DATA
  ...
  If @TimeSort = 1
    If @MasterUnit Is NULL
      BEGIN
          SELECT D.Detail_Id
               , Start_Time = Case When D.Crew_Start Is NULL Then dbo.fnServer_CmnConvertFromDbTime(D.Start_Time,@InTimeZone)
                                   When D.Start_Time <= D.Crew_Start Then dbo.fnServer_CmnConvertFromDbTime(D.Crew_Start,@InTimeZone) Else dbo.fnServer_CmnConvertFromDbTime(D.Start_Time,@InTimeZone) End
               , End_Time   = Case When D.Crew_End Is NULL Then  dbo.fnServer_CmnConvertFromDbTime(D.End_Time,@InTimeZone)
                                   When D.End_Time >= D.Crew_End Then  dbo.fnServer_CmnConvertFromDbTime(D.Crew_End,@InTimeZone) ELSE dbo.fnServer_CmnConvertFromDbTime(D.End_Time,@InTimeZone) END
               , D.Duration
               , D.Uptime
               , P.Prod_Code
...
UPDATE #TopNDR SET Start_Time = n.Endtime,
 	  	  	  	  	 NPT = 1
FROM #TopNDR  JOIN @Periods_NPT1 n ON (Start_Time > n.StartTime AND End_time > n.EndTime AND Start_Time < n.EndTime)
-- Case 2 :  Downtime    St---------------------End
-- 	  	  	  NPT 	  	  	  	  	 St--------------End
UPDATE #TopNDR SET End_Time = n.Starttime,
 	  	  	  	    NPT = 1
FROM 	 #TopNDR 	  	    
JOIN @Periods_NPT1 n ON (Start_Time < n.StartTime AND End_Time < n.Endtime AND End_Time > n.StartTime)
...
UPDATE #TopNDR SET Start_Time = End_Time,
 	  	  	  	  	 NPT = 1
FROM 	 #TopNDR  	  	    
JOIN @Periods_NPT1 n ON( (Start_Time BETWEEN n.StartTime AND n.EndTime) AND (End_time BETWEEN n.StartTime AND n.EndTime))
Update #TopNDR Set Duration =DateDiff(ss,Start_Time,End_Time)/60.0
-- Case 4 :  Downtime   St-----------------------End
-- 	  	  	  NPT 	  	    St-----------------End
UPDATE #TopNDR  SET NPT = 1
FROM #TopNDR  JOIN @Periods_NPT1 n ON( (n.StartTime BETWEEN Start_Time AND End_Time) AND (n.Endtime BETWEEN Start_Time AND End_Time))
    ...
    If @MasterUnit Is NULL
      BEGIN
          SELECT D.Detail_Id
               , Start_Time = Case When D.Crew_Start Is NULL Then dbo.fnServer_CmnConvertFromDbTime(D.Start_Time,@InTimeZone)
                                   When D.Start_Time <= D.Crew_Start Then dbo.fnServer_CmnConvertFromDbTime(D.Crew_Start,@InTimeZone) Else dbo.fnServer_CmnConvertFromDbTime(D.Start_Time,@InTimeZone) End
               , End_Time   = Case When D.Crew_Start Is NULL Then  dbo.fnServer_CmnConvertFromDbTime(D.End_Time,@InTimeZone)
                                   When D.End_Time >= D.Crew_End Then  dbo.fnServer_CmnConvertFromDbTime(D.Crew_End,@InTimeZone) ELSE dbo.fnServer_CmnConvertFromDbTime(D.End_Time,@InTimeZone) END
               , D.Duration
               , D.Uptime
