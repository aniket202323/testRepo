-- DESCRIPTION: spXLA_DowntimeDetail_NoProduct_New_NPT is modified from spXLA_DowntimeDetail_NoProduct. Changes includes
-- (a) new field name (Unit instead of Line)
-- (b) Uptime added.
-- mt/7-1-2002
CREATE PROCEDURE dbo.[spXLA_DowntimeDetail_NoProduct_New_NPT_Bak_177]
 	   @Start_Time 	 datetime
 	 , @End_Time 	 datetime
...
If @MasterUnit Is NULL
  BEGIN
    INSERT INTO #TopNDR (Detail_Id, Start_Time, End_Time, Duration, Uptime, SourcePU, MasterUnit, R1_Id, R2_Id, R3_Id, R4_Id, Fault_Id, Status_Id)
      SELECT  D.TEDEt_Id, D.Start_Time, D.End_Time, D.Duration, D.Uptime, D.Source_PU_Id, PU.PU_Id, D.Reason_Level1, D.Reason_Level2,D.Reason_Level3,D.Reason_Level4, TEFault_Id, TEStatus_Id
