...
CREATE PROCEDURE dbo.spXLA_EventGenealogy
 	 @Event_Id 	  	 Int
...
If @RelationSought = @Child -- we seek this event's child(ren)
  BEGIN
    SELECT e.Event_Num
         , e.Event_Id
         , [Start_Time] = COALESCE(e.Start_Time at time zone @DBTz at time zone @InTimeZone, DATEADD(ss, -1, e.TimeStamp at time zone @DBTz at time zone @InTimeZone))
         ...
         , [Event_Component_Timestamp]           = C.TimeStamp at time zone @DBTz at time zone @InTimeZone
         , [Event_Component_Start_Time]          = C.Start_Time at time zone @DBTz at time zone @InTimeZone
         , [Event_Component_Entry_On]            = C.Entry_On at time zone @DBTz at time zone @InTimeZone
