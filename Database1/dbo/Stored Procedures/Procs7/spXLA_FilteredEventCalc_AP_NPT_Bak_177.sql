-- 
-- spXLA_FilteredEventCalc_AP_NPT() is derived from spXLAEventCalc_AP. Defect #24497:mt/9-17-2002:added Crew,Shift
-- ECR #28662: mt/8-20-2004: replace unreliable Case statement when NULL with COALESCE 
CREATE PROCEDURE dbo.[spXLA_FilteredEventCalc_AP_NPT_Bak_177]
...
--
CREATE TABLE #Event_Details (PU_Id Int, Event_Id Int, TimeStamp DateTime, Start_Time DateTime, Dimension Real NULL)
CREATE TABLE #EventsTotal ( PU_id int,Event_id int, TimeStamp DateTime, Start_Time DateTime , Total int NULL, Dim real, Diff real) -- NPT
...
RETRIEVE_WITH_APPLIED_PRODUCT_FILTER:
  ...
  SELECT @Min     = MIN(ed.Dim)
       ...
       , @Count   = COUNT(ed.Dim)- @NPTCOUNT 
 	    , @Average = @Total / @Count
    FROM #EventsTotal ed
    JOIN #Applied_Products ap On ap.Event_Id = ed.Event_Id AND ap.PU_Id = ed.PU_Id
  If @ExtraCalcs = 1  -- (Applied Products)
    BEGIN
      DECLARE TCursor INSENSITIVE CURSOR 
          FOR (SELECT ed.PU_Id, e.TimeStamp, ed.Dimension
                 FROM #Event_Details ed 
                 JOIN Events_With_StartTime e on e.Event_Id = ed.Event_Id
                 JOIN #Applied_Products ap On ap.Event_Id = ed.Event_Id AND ap.PU_Id = ed.PU_Id
              )
