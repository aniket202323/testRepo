...
CREATE PROCEDURE dbo.spXLA_FilteredTestsData_AP
 	   @Var_Id 	  	 Integer
...
HASCREW_NOSHIFT_ORIGINAL_RESULTSET:
  If @TimeSort = 1 
        SELECT [Result_On] = t.Result_On at time zone @DBTz at time zone @InTimeZone, [Entry_On] = t.Entry_On at time zone @DBTz at time zone @InTimeZone, t.Canceled, C.Crew_Desc, C.Shift_Desc, t.Comment_Id
             , [Result] = Case When @DecimalChar <> '.' AND @Data_Type_Id = 2 THEN REPLACE(t.Result, '.', @DecimalChar) Else t.Result End
...
NOCREW_NOSHIFT_APPLIED_RESULTSET:
  ...
  Else
      SELECT p.Prod_Code, Applied_Prod_Code = p2.Prod_Code, C.Crew_Desc, C.Shift_Desc
           , [Result_On] = t.Result_On at time zone @DBTz at time zone @InTimeZone, [Entry_On] = t.Entry_On at time zone @DBTz at time zone @InTimeZone, t.Canceled, e.Event_Id, e.Event_Num, Event_Status = s.ProdStatus_Desc, t.Comment_Id
           , [Result] = Case When @DecimalChar <> '.' AND @Data_Type_Id = 2 THEN REPLACE(t.Result, '.', @DecimalChar) Else t.Result End
           , Data_Type_Id = @Data_Type_Id, Event_Type = @Event_Type, Pu_Id = @Pu_Id
        FROM @MyTests t
        JOIN #Applied_Products ap ON ap.Start_Time < t.Result_On AND ap.End_Time >= t.Result_On AND t.Canceled = 0
