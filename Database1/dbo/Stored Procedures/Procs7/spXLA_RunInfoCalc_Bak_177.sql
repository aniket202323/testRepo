--spXLA_RunInfoCalc replaces spXLARunInfoCalc_New. ECR #25128: mt/3-9-2003: Changes to include handling of duplicate PU_Desc
--
CREATE PROCEDURE dbo.[spXLA_RunInfoCalc_Bak_177] 
 	   @Start_Time  	 DateTime
 	 , @End_Time  	 DateTime
 	 , @Pu_Id  	 Integer
...
Else --we have PU_Desc
  BEGIN
    ...
    SELECT @ProductionUnitCount = @@ROWCOUNT
    If @ProductionUnitCount <> 1
      BEGIN
        If @ProductionUnitCount = 0 
          SELECT [ReturnStatus] = -100 	  	 --Specified Prod Unit NOT FOUND
        Else
          SELECT [ReturnStatus] = -103 	  	 --DUPLICATE FOUND; for Prod Unit Description
        --EndIf
        RETURN
      END
    --EndIf: 
...
If @qType = @NoProductNoTime
  BEGIN
    SELECT @SQLString = @SQLString + 
 	 'DECLARE spXLARunInfoCalc_TCursor  CURSOR Global SCROLL
...
Else If @qType = @GroupAndProperty
  ...
  END 	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	 
--EndIf @qType ...
OPEN spXLARunInfoCalc_TCursor  
SELECT @FetchCount = 0
SELECT @SumDurationWeighted = 0
SELECT @TotalDuration = 0
AVERAGE_LOOP:
--                                             1           2             3           4              
    FETCH NEXT FROM spXLARunInfoCalc_TCursor INTO @@Start_Time, @@Duration, @@In_Warning, @@In_Limit, @@Conf_Index
    If (@@Fetch_Status = 0)
 	 BEGIN
...
STD_DEV_LOOP:
    If @FetchCount = 0
 	 FETCH FIRST FROM spXLARunInfoCalc_TCursor INTO @@Start_Time, @@Duration, @@In_Warning, @@In_Limit, @@Conf_Index
    Else
        --                                           1           2             3           4              
        FETCH NEXT FROM spXLARunInfoCalc_TCursor INTO @@Start_Time, @@Duration, @@In_Warning, @@In_Limit, @@Conf_Index
    --EndIf
...
CLOSE spXLARunInfoCalc_TCursor
DEALLOCATE spXLARunInfoCalc_TCursor
