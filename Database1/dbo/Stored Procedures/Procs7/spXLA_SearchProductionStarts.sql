-- spXLA_SearchProductionStarts() is modified from spXLASearchProductionStarts_AP. ECR #25128: mt/3-11-2003: Changed to handle
-- duplicate PU_Desc. MSI GBDB does not enforce unique PU_Desc in the entire database system.
--
-- NOTE: Although the stored procedure code appears to be handling MasterUnit_Id = NULL; in practice ProficyAddIn client NEVER
-- call the stored procedure without a production unit. ProficyProduction client actually error out if production unit is not 
-- specified: this client requirement goes all the way back to PrfXla.xla v2.1B49
--
CREATE PROCEDURE dbo.spXLA_SearchProductionStarts
 	   @Start_Time  	  	 DateTime
 	 , @End_Time  	  	 DateTime
  ...
  BEGIN
    If @MasterUnit_ID Is NULL
      BEGIN
        If @TimeSort = 1
          BEGIN
              SELECT Production_Unit = pu.pu_desc,Start_Time = ps.Start_Time at time zone @DBTz at time zone @InTimeZone,End_Time = ps.End_Time at time zone @DBTz at time zone @InTimeZone, p.Prod_Code
...
Else 	  	  	  	  	 --Some product grouping exist
  BEGIN
    ...
    If @MasterUnit_ID Is NULL
      BEGIN
        If @TimeSort = 1 
          SELECT Production_Unit = pu.pu_desc,Start_Time = ps.Start_Time at time zone @DBTz at time zone @InTimeZone,End_Time = ps.End_Time at time zone @DBTz at time zone @InTimeZone, pt.Prod_Code
            FROM production_starts ps
    ...
    BEGIN
      If @MasterUnit_ID Is NULL
        BEGIN
          If @TimeSort = 1
            BEGIN
                SELECT Production_Unit = pu.pu_desc,Start_Time = ps.Start_Time at time zone @DBTz at time zone @InTimeZone,End_Time = ps.End_Time at time zone @DBTz at time zone @InTimeZone
                  FROM production_starts ps
                  JOIN prod_units pu on pu.Pu_Id = ps.Pu_Id
                 WHERE (Start_Time BETWEEN @Start_Time AND @End_Time) 
                    --OR (End_Time BETWEEN @Start_Time AND @End_Time) 
...
TOP_OF_FETCH_LOOP:
  ...
  If @TimeSort = 1 --Asc
    BEGIN
        SELECT Production_Unit = pu.Pu_Desc, ap.Start_Time at time zone @DBTz at time zone @InTimeZone, ap.End_Time at time zone @DBTz at time zone @InTimeZone, p.Prod_Code, Applied_Prod_Code = p2.Prod_Code
...
RETRIEVE_APPLIED_PRODUCT_FILTER_WITHOUT_PRODUCT_CODE:
  ...
  If @TimeSort = 1 --Asc
    BEGIN
       --Select only rows that fall within specified time range  
        SELECT Production_Unit = pu.Pu_Desc, ap.Start_Time at time zone @DBTz at time zone @InTimeZone, ap.End_Time at time zone @DBTz at time zone @InTimeZone
          FROM #Applied_Products ap
          JOIN Prod_Units pu ON pu.Pu_Id = ap.Pu_Id
         WHERE ap.Start_Time BETWEEN @Start_Time AND @End_Time
            OR ap.End_Time BETWEEN @Start_Time AND @End_Time
