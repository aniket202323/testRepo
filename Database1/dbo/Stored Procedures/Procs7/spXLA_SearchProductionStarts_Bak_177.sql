...
--
CREATE PROCEDURE dbo.[spXLA_SearchProductionStarts_Bak_177]
 	   @Start_Time  	  	 DateTime
 	 , @End_Time  	  	 DateTime
...
TOP_OF_FETCH_LOOP:
  ...
  If @TimeSort = 1 --Asc
    BEGIN
        SELECT Production_Unit = pu.Pu_Desc, dbo.fnServer_CmnConvertFromDbTime(ap.Start_Time,@InTimeZone), dbo.fnServer_CmnConvertFromDbTime(ap.End_Time,@InTimeZone), p.Prod_Code, Applied_Prod_Code = p2.Prod_Code
...
RETRIEVE_APPLIED_PRODUCT_FILTER_WITHOUT_PRODUCT_CODE:
  ...
  If @TimeSort = 1 --Asc
    BEGIN
       --Select only rows that fall within specified time range  
        SELECT Production_Unit = pu.Pu_Desc, dbo.fnServer_CmnConvertFromDbTime(ap.Start_Time,@InTimeZone), dbo.fnServer_CmnConvertFromDbTime(ap.End_Time,@InTimeZone)
          FROM #Applied_Products ap
          JOIN Prod_Units pu ON pu.Pu_Id = ap.Pu_Id
         WHERE ap.Start_Time BETWEEN @Start_Time AND @End_Time
            OR ap.End_Time BETWEEN @Start_Time AND @End_Time
