-- DESCRIPTION: spXLA_UserDefinedEventSUMM_NoProduct. ECR #27888: mt/4-28-2004
--
CREATE PROCEDURE dbo.spXLA_UserDefinedEventSUMM_NoProduct
 	   @Start_Time 	  	 DateTime
 	 , @End_Time 	  	 DateTime
 	 , @PU_Id                Int 
...
--  UPDATE #TempUDE SET Reason_Name = dbo.fnDBTranslate(@LangId, 38333, 'Unspecified') WHERE Reason_Name Is NULL
  ...
  INSERT INTO #MyReport( ReasonName , NumberOfOccurances, TotalReasonMinutes ,            AvgReasonMinutes,                  TotalUserDefinedEventMinutes,  TotalOperatingMinutes)
    SELECT               Reason_Name, COUNT(Duration),    Total_Duration = SUM(Duration), (SUM(Duration) / COUNT(Duration)), @TotalUserDefinedEventMinutes, @Start_End_Time_Span
      FROM #TempUDE
