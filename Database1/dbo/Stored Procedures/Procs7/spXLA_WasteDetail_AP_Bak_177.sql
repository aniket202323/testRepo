-- DESCRIPTION: spXLA_WasteDetail_AP handles original and applied product filter. Any product (no product 
-- information specified) is not a valid option for applied product. No product specified is handled by a separate stored
-- procedure (spXLA_WasteDetail_AP_NoProduct). MT/3-26-2002
-- ECR #29673: mt/5-3-2005 -- Amount as additional sort criteria in return result set 
CREATE PROCEDURE dbo.[spXLA_WasteDetail_AP_Bak_177]
 	   @Start_Time  	  	 DateTime
 	 , @End_Time  	  	 DateTime
 	 , @Pu_Id  	  	 Int
 	 , @SelectSource  	 Int
...
CREATE TABLE #Applied_Products(Pu_Id Int, Event_Id Int, Event_Num Varchar(25), Prod_Id Int, Applied_Prod_Id Int NULL, TimeStamp DateTime)
...
INSERT INTO #TopNDR (Detail_Id, TimeStamp, Amount, SourcePU, R1_Id, R2_Id, R3_Id, R4_Id, Type_Id, Meas_Id, Prod_Id, EventBased, EventNumber)
  SELECT D.WED_Id, EV.TimeStamp, D.Amount, D.Source_Pu_Id, D.Reason_Level1, D.Reason_Level2, D.Reason_Level3, D.Reason_Level4, D.WET_Id, D.WEMT_Id
...
DO_FILTER_BY_APPLIED_PRODUCT:
  ...
  INSERT INTO #TopNDR (Detail_Id, TimeStamp, Amount, SourcePU, R1_Id, R2_Id, R3_Id, R4_Id, Type_Id, Meas_Id, Prod_Id, Applied_Prod_Id, EventBased, EventNumber)
    SELECT D.WED_Id, ap.TimeStamp, D.Amount, D.Source_Pu_Id, D.Reason_Level1, D.Reason_Level2, D.Reason_Level3, D.Reason_Level4, D.WET_Id, D.WEMT_Id
