...
Create Procedure dbo.[spXLACapturedData_Bak_177]
 	 @varId integer, 
 	 @sTime datetime,
 	 @eTime datetime,
 	 @puId integer, 
...
-- MSi/MT/2-28-2001 
  ...
  SELECT @tmpSQL2 = ''
  SELECT @tmpSQL3 = ''
  Select @tmpSQL1 = 'select timestamp = dbo.fnServer_CmnConvertFromDbTime(ds.timestamp, ''' + Isnull(@InTimeZone,'Null') + '''), ds.prod_id, dsd.* from gb_dset ds  WITH (index(dset_by_pu)) left outer ' + 
