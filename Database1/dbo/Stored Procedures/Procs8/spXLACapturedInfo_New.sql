--spXLACapturedInfo_New (mt/1-3-2002) is modified from spXLACapturedInfo_Expand. Changes are
--  (1) Accept either ID or Description for production unit. Verify Production Unit information on server
--  (2) Include product code in resultSet, only when asked for.
--
CREATE PROCEDURE dbo.spXLACapturedInfo_New 
 	   @Start_Time  	  	 DateTime
 	 , @End_Time  	  	 DateTime
 	 , @Pu_Id  	  	 Integer
...
Else
  ...
  END
--EndIf
If @ProductionUnitCount = 0 RETURN 	 --RecordSet field count = 0 indicates "Production Unit specified NOT found"
...
If @QueryType = @NoProductInfoNoEndTime
  BEGIN 	 
    If @TimeSort = 1 	 --Ascending
 	     SELECT DSet_Id,Comment_Id,Operator,Prod_Id,PU_Id,[Timestamp] = [Timestamp] at time zone @DBTz at time zone @InTimeZone
...
DO_PRODUCT_CODE_SQL:
  ...
  If @QueryType = @NoProductInfoNoEndTime
    BEGIN
      If @TimeSort = 1  --Ascending
          SELECT p.Prod_Code,ds.DSet_Id,ds.Comment_Id,ds.Operator,ds.Prod_Id,ds.PU_Id,[Timestamp] = ds.[Timestamp] at time zone @DBTz at time zone @InTimeZone
            FROM gb_dset ds
