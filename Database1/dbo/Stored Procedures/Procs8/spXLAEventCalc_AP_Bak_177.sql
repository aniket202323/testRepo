-- mt/5-10-2002 
-- spXLAEventCalc_AP() 
--   (1) spXLAEventCalc_AP accepts variable ID or description as input
--   (2) Ability to filter by applied product or original product
--
CREATE PROCEDURE dbo.[spXLAEventCalc_AP_Bak_177]
 	   @PL_Id 	  	 Int 	  	 --Line
 	 , @PL_Desc 	  	 Varchar(50)
 	 , @PU_Id                Int
 	 , @PU_Desc              Varchar(50)
...
Else --some unit is selected
  BEGIN
    ...
    If @PU_Desc Is NOT NULL
      BEGIN
        SELECT @PU_Id = Case Master_Unit When NULL Then PU_Id Else Master_Unit End
...
RETRIEVE_WITH_APPLIED_PRODUCT_FILTER:
  ...
  If @ExtraCalcs = 1  -- (Applied Products)
    BEGIN
      DECLARE TCursor INSENSITIVE CURSOR 
          FOR (SELECT ed.PU_Id, ed.TimeStamp, ed.Dimension
                 FROM #Event_Details ed 
                 JOIN #Applied_Products ap On ap.Event_Id = ed.Event_Id AND ap.PU_Id = ed.PU_Id
              )
          FOR READ ONLY
...
RETRIEVE_RECORDSET_WITHOUT_PRODUCTS:
  ...
  If @ExtraCalcs = 1  --(No product code ever requested)
    BEGIN
      DECLARE TCursor INSENSITIVE CURSOR 
          FOR ( SELECT e.PU_Id, e.TimeStamp, ed.Dimension FROM #Event_Details ed JOIN Events e on e.Event_Id = ed.Event_Id) 
          FOR READ ONLY
      GOTO OPEN_CURSOR_AND_PROCESS_EXTRACALC_STUFF
    END
  --EndIf 
