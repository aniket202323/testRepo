Create Procedure dbo.[spXLALastRunInfo_Bak_177] @puid integer, 
 	  	  	 @prodid integer,
 	  	  	 @groupid integer,
 	  	  	 @propid integer,
 	  	  	 @charid integer
...
-- MSi/MT/2-28-2001 
  SELECT @tmpSQL1 = ''
  SELECT @tmpSQL2 = ''
  Select @tmpSQL1 = 'select RSum_Id,Comment_Id,Conf_Index,Duration,End_Time = dbo.fnServer_CmnConvertFromDbTime(End_Time,''' + isnull(@InTimeZone,'null') + '''),
 	  	  	 In_Limit,In_Warning,Prod_Id,PU_Id,Start_Time = dbo.fnServer_CmnConvertFromDbTime(Start_Time,''' + isnull(@InTimeZone,'null') + ''')  from gb_rsum WITH (INDEX(rsum_by_pu)) where pu_id = ' + Convert(varchar(50),@puid) +
