Create Procedure dbo.[spXLARunDataCalc_Bak_177] 
 	   @varid integer
 	 , @stime datetime
 	 , @etime datetime
 	 , @puid integer
...
SELECT @etime = dbo.fnServer_CmnConvertToDBTime(@etime,@InTimeZone)
  Select @tmpSQL1 = 'select dbo.fnServer_CmnConvertFromDbTime(RS.start_time,''' + Isnull(@InTimeZone,'Null') + '''), RS.duration, RSD.* from gb_rsum RS WITH (INDEX(rsum_by_product)) ' +
