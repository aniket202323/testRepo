--spXLARunDataCalc_New replaces spXLA_RunDataCalculations. Changes are:
--  (1) spXLARunDataCalc_New does internal lookup for Pu_Id & Data_Type_Id based on input variable
--  (2) Variable can be ID or description. spXLARunDataCalc_New verifies validity of variable input.
--
CREATE PROCEDURE dbo.[spXLARunDataCalc_New_Bak_177] 
 	   @Var_Id  	 Integer
...
Else
  ...
  END
--EndIf
--Report single field ReturnStatus to caller.
If @VariableCount = 0
  BEGIN
    SELECT ReturnStatus = -10 	  	 --Indicate "Variable specified NOT found"
    RETURN
  END
...
If @qType = @AnyProductNoEndTime --  {[@ProdId, @GroupId, @PropId, @CharId] are NULL}, @End_Time NULL
  BEGIN
    SELECT @SQLString = @SQLString + 
 	 'DECLARE spXLARunDataCalc_NewTCursor CURSOR Global SCROLL 
...
Else If @qType = @GroupAndProperty 	  	  	 -- @Group_Id NOT NULL, @Prop_Id NOT NULL, @End_Time NOT NULL
  ...
  END
--EndIf @QueryType..
OPEN spXLARunDataCalc_NewTCursor  
SELECT @FetchCount = 0
SELECT @SumDurationWeighted = 0
SELECT @TotalDuration = 0
AVERAGE_LOOP:
--                                                         1        2            3           4              5         6      7          8          9
    FETCH NEXT FROM spXLARunDataCalc_NewTCursor INTO @@Start_Time, @@Duration, @@Value, @@In_Warning, @@In_Limit, @@Conf_Index,  @@StdDev, @@Cpk, @@Minimum, @@Maximum, @@Num_Values
    If (@@Fetch_Status = 0)
 	 BEGIN
...
STD_DEV_LOOP:
    If @FetchCount = 0
 	 FETCH FIRST FROM spXLARunDataCalc_NewTCursor INTO @@Start_Time, @@Duration, @@Value, @@In_Warning, @@In_Limit, @@Conf_Index,  @@StdDev, @@Cpk, @@Minimum, @@Maximum, @@Num_Values
    Else
--                                                         1        2            3           4              5         6      7          8          9
 	 FETCH NEXT FROM spXLARunDataCalc_NewTCursor INTO @@Start_Time, @@Duration, @@Value, @@In_Warning, @@In_Limit, @@Conf_Index,  @@StdDev, @@Cpk, @@Minimum, @@Maximum, @@Num_Values
    --EndIf
...
CLOSE spXLARunDataCalc_NewTCursor
DEALLOCATE spXLARunDataCalc_NewTCursor
