CREATE PROCEDURE dbo.spXLARunData_Expand 
 	   @Var_Id  	 Integer
 	 , @Start_Time 	 DateTime
 	 , @End_Time  	 DateTime
 	 , @Pu_Id  	 Integer
...
if @QueryType = 17 	  	 --  {[@ProdId,@GroupId,@PropId,@CharId] Null}, @End_Time Null, @TimeSort = 1
    BEGIN
 	 SELECT  [Start_Time] = rs.Start_Time at time zone @DBTz at time zone @InTimeZone, [End_Time] = rs.End_Time at time zone @DBTz at time zone @InTimeZone, rs.prod_id, rsd.* 
 	   FROM  gb_rsum rs 
