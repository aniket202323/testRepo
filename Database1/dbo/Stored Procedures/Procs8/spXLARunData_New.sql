--spXLARunData_New is modified from spXLARunData_Expand. Changes are 
--  (1) Include internal lookup, Pu_Id & Data_Type_Id, so XLA doesn't have to do this
--  (2) Include Prod_Code in the resultSet, so XLA doesn't have to do further look up for product code
--  (3) Join to get product only when needed.
--mt/1-2-2002
--
CREATE PROCEDURE dbo.spXLARunData_New 
...
If @QueryType = @NOProductNOEndAscend  	  	  	 --17
    BEGIN
 	 SELECT  [Start_Time] = rs.Start_Time at time zone @DBTz at time zone @InTimeZone, [End_Time] = rs.End_Time at time zone @DBTz at time zone @InTimeZone, rs.prod_id, 
    rsd.Conf_Index
    ,rsd.Cpk
    ,rsd.In_Limit
    ,rsd.In_Warning
...
DO_INCLUDE_PRODUCT_CODE_SQL:
  ...
  If @QueryType = @NOProductNOEndAscend       --17
    BEGIN
        SELECT  [Start_Time] = rs.Start_Time at time zone @DBTz at time zone @InTimeZone, [End_Time] = rs.End_Time at time zone @DBTz at time zone @InTimeZone, rs.prod_id, p.Prod_Code, 
        rsd.Conf_Index
        ,rsd.Cpk
        ,rsd.In_Limit
        ,rsd.In_Warning
