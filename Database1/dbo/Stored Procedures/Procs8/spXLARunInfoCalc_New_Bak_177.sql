--spXLARunInfoCalc_New replaces spXLA_RunInfoCalculations. Changes are
-- (1) Production Unit input can be ID or Description. Stored procedure verifies validy of production unit for Proficy Add-In
-- (2) 
--
CREATE PROCEDURE dbo.[spXLARunInfoCalc_New_Bak_177] 
 	   @Start_Time  	 DateTime
 	 , @End_Time  	 DateTime
 	 , @Pu_Id  	 Integer
...
Else
  ...
  END
--EndIf
If @ProductionUnitCount = 0 RETURN 	 --zero field count in recordSet indicate "production unit specified NOT found"
...
If @qType = @NoProductNoTime
  BEGIN
    SELECT @SQLString = @SQLString + 
 	 'DECLARE spXLARunInfoCalc_New_TCursor CURSOR Global SCROLL  
...
Else If @qType = @GroupAndProperty
  ...
  END 	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	 
--EndIf @qType ...
OPEN spXLARunInfoCalc_New_TCursor  
SELECT @FetchCount = 0
SELECT @SumDurationWeighted = 0
SELECT @TotalDuration = 0
AVERAGE_LOOP:
--                                             1           2             3           4              
    FETCH NEXT FROM spXLARunInfoCalc_New_TCursor INTO @@Start_Time, @@Duration, @@In_Warning, @@In_Limit, @@Conf_Index
    If (@@Fetch_Status = 0)
 	 BEGIN
...
STD_DEV_LOOP:
    If @FetchCount = 0
 	 FETCH FIRST FROM spXLARunInfoCalc_New_TCursor INTO @@Start_Time, @@Duration, @@In_Warning, @@In_Limit, @@Conf_Index
    Else
        --                                           1           2             3           4              
        FETCH NEXT FROM spXLARunInfoCalc_New_TCursor INTO @@Start_Time, @@Duration, @@In_Warning, @@In_Limit, @@Conf_Index
    --EndIf
...
CLOSE spXLARunInfoCalc_New_TCursor
DEALLOCATE spXLARunInfoCalc_New_TCursor
