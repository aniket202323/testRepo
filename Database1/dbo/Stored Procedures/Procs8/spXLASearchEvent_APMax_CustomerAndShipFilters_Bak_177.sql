/* NOTE ON spXLASearchEvent_APMax: Derived from spXLASearchEvent, this stored procedure adds the ability to
   ...
   (4) spXLASearchEvent_APMax_CustomerAndShipFilters 	  	 'This stored Procedure; 1-22-2002
   ...
   (8) spXLASearchEvent_APMax_PlantCustomerAndShip
*/
-- USAGE: 
-- spXLASearchEvent_APMax_CustomerAndShipFilters  should be called only when two filters are specified: Customer_Code
-- & Shipment_Number filters. MSi/mt/12-07-2001
--
CREATE PROCEDURE dbo.[spXLASearchEvent_APMax_CustomerAndShipFilters_Bak_177] 
  ...
  If @MyType = @NoStringHasStatusAsc --1
    BEGIN
        SELECT Primary_Event_Num = e.Event_Num
          ...
          LEFT OUTER JOIN Customer_Order_Line_Items l ON l.Order_Line_Id = ed.Order_Line_Id      --22Jan2002;mt
          JOIN  Customer_Orders co ON co.Order_Id = l.Order_Id                                   --22Jan2002;mt
          JOIN Customer c ON c.Customer_Id = co.Customer_Id AND c.Customer_Code = @Customer_Code --22Jan2002;mt
          LEFT OUTER JOIN Shipment_Line_Items sl ON sl.Shipment_Item_Id = ed.Shipment_Item_Id           --2002Jan22;mt
          JOIN Shipment sh ON sh.Shipment_Id = sl.Shipment_Id AND sh.Shipment_Number = @Shipment_Number --2002Jan22;mt
