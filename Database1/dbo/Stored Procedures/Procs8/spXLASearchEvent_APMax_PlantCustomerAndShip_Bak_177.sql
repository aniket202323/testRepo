/* NOTE ON spXLASearchEvent_APMax: Derived from spXLASearchEvent, this stored procedure adds the ability to
   ...
   NOT to add more branches of SQL Statement into spXLASearchEvent_APMax but to expand into 8 stored procedures listed 
   below. Future bug fix to core code, if any, should be done in all 8 stored procedures.  
   The 8 stored procedures are:
   (1) spXLASearchEvent_APMax 	  	  	  	  	 'Master copy
   (2) spXLASearchEvent_APMax_ShipFilterOnly
   (3) spXLASearchEvent_APMax_CustomerFilterOnly 	  	 
   (4) spXLASearchEvent_APMax_CustomerAndShipFilters
   (5) spXLASearchEvent_APMax_PlantFilterOnly 	  	  	 
   (6) spXLASearchEvent_APMax_PlantAndShipFilters
   (7) spXLASearchEvent_APMax_PlantAndCustomerFilters
   (8) spXLASearchEvent_APMax_PlantCustomerAndShip 	  	 'This stored Procedure; mt/1-23-2002
*/
-- USAGE: 
-- spXLASearchEvent_APMax should be called only when maximum join condition is met and Customer_Code is supplied as 
-- the only filter. MSi/mt/1-23-2002
--
CREATE PROCEDURE dbo.[spXLASearchEvent_APMax_PlantCustomerAndShip_Bak_177] 
...
Else --@MasterUnit NOT NULL
  ...
  END
--EndIf @MasterUnit...
GOTO UPDATE_TEMP_EVENTS_AND_GET_RESULTSET
...
DO_APPLIED_PRODUCT_FILTER_STUFF:
  ...
  --EndIf @MasterUnit...
  GOTO UPDATE_TEMP_EVENTS_AND_GET_RESULTSET
UPDATE_TEMP_EVENTS_AND_GET_RESULTSET:
  ...
  If @MyType = @NoStringHasStatusAsc --1
    BEGIN
         SELECT Primary_Event_Num = e.Event_Num
           ...
           LEFT OUTER JOIN Customer_Order_Line_Items l ON l.Order_Line_Id = ed.Order_Line_Id                    --23Jan2002;mt
           JOIN Customer_Orders co ON co.Order_Id = l.Order_Id AND co.Plant_Order_Number = @Plant_Order_Number --23Jan2002;mt
           JOIN Customer c ON c.Customer_Id = co.Customer_Id AND c.Customer_Code = @Customer_Code               --23Jan2002;mt
           LEFT OUTER JOIN Shipment_Line_Items sl ON sl.Shipment_Item_Id = ed.Shipment_Item_Id           --2002Jan23;mt
           JOIN Shipment sh ON sh.Shipment_Id = sl.Shipment_Id AND sh.Shipment_Number = @Shipment_Number --2002Jan23;mt
