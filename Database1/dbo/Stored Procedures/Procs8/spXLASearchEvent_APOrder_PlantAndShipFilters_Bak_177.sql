...
*/
-- USAGE:
-- spXLASearchEvent_APOrder_PlantAndShipFilters should be called only when none of the filters are supplied (except 
-- for products), the Order-Join condtions are met, and Plant_Order_Number & Shipment_Number filters are specified. 
-- MSi/mt/1-24-2002
--
CREATE PROCEDURE dbo.[spXLASearchEvent_APOrder_PlantAndShipFilters_Bak_177] 
  ...
  If @MyType = @NoStringHasStatusAsc --1
    BEGIN
        SELECT Primary_Event_Num = e.Event_Num
          ...
          LEFT OUTER JOIN Shipment_Line_Items sl ON sl.Shipment_Item_Id = ed.Shipment_Item_Id             --2002Jan24;mt
          JOIN Shipment sh ON sh.Shipment_Id = sl.Shipment_Id AND sh.Shipment_Number = @Shipment_Number   --2002Jan24;mt
