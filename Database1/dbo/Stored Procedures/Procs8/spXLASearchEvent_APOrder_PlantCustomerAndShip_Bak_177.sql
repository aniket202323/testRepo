/* NOTE ON spXLASearchEvent_APOrder: Derived from spXLASearchEvent_APMax (<spXLASearchEvent). The suffix _AP indicate
   stored procedures with the ability to filter by "Applied Product" in addition to filtering by "Original Product" 
   ...
   MSi/mt/1-22-2002
   MSi/mt/1-22-2002; spXLASearchEvent_APOrder is for event search with the join condition to retrieve order information.
   ...
   We chose NOT to add more branches of SQL Statements to spXLASearchEvent_APOrder, but rather to expand 
   ...
   The 8 stored procedures are:
    (1) spXLASearchEvent_APOrder 	  	  	  	 'Master Copy; This stored procedure
    (2) spXLASearchEvent_APOrder_ShipmentFilterOnly
    (3) spXLASearchEvent_APOrder_CustomerFilterOnly
    (4) spXLASearchEvent_APOrder_CustomerAndShipFilters
    (5) spXLASearchEvent_APOrder_PlantFilterOnly
    (6) spXLASearchEvent_APOrder_PlantAndShipFilters
    (7) spXLASearchEvent_APOrder_PlantAndCustomerFilters
    (8) spXLASearchEvent_APOrder_PlantCustomerAndShip
*/
-- USAGE:
-- spXLASearchEvent_APOrder_PlantCustomerAndShip should be called only the Order-Join condtions are met, 
-- and Plant_Order_Number, Customer_Code, and Shipment_Number filters are specified. MSi/mt/1-24-2002
--
CREATE PROCEDURE dbo.[spXLASearchEvent_APOrder_PlantCustomerAndShip_Bak_177] 
...
CREATE TABLE #Events (Event_Num Varchar(25), Event_Id Int, Pu_Id Int, Prod_Id Int, Start_Time DateTime, End_Time DateTime
  ...
  If @MyType = @NoStringHasStatusAsc --1
    BEGIN
        SELECT Primary_Event_Num = e.Event_Num
          ...
          LEFT OUTER JOIN Customer_Order_Line_Items l ON l.Order_Line_Id = ed.Order_Line_Id                    --24Jan02;mt
          JOIN Customer_Orders co ON co.Order_Id = l.Order_Id AND co.Plant_Order_Number = @Plant_Order_Number  --24Jan02;mt
          JOIN Customer c ON c.Customer_Id = co.Customer_Id AND c.Customer_Code = @Customer_Code               --24Jan02;mt
          LEFT OUTER JOIN Shipment_Line_Items sl ON sl.Shipment_Item_Id = ed.Shipment_Item_Id              --24Jan02;mt
          JOIN Shipment sh ON sh.Shipment_Id = sl.Shipment_Id AND sh.Shipment_Number = @Shipment_Number    --24Jan02;mt
