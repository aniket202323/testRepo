/* NOTE ON spXLASearchEvent_APOrder: Derived from spXLASearchEvent_APMax (<spXLASearchEvent). The suffix _AP indicate
   ...
   The 8 stored procedures are:
    (1) spXLASearchEvent_APOrder 	  	  	  	 'Master Copy
    (2) spXLASearchEvent_APOrder_ShipmentFilterOnly 	  	 'This stored procedure; mt/1-23-2002
...
*/
-- USAGE:
-- spXLASearchEvent_APOrder_ShipmentFilterOnly should be called only when none of the filters are supplied (except 
-- for products), and the Order-Join condtions is met AND Shipment_Number filter is specified. MSi/mt/1-23-2002
--
CREATE PROCEDURE dbo.[spXLASearchEvent_APOrder_ShipmentFilterOnly_Bak_177] 
  ...
  If @MyType = @NoStringHasStatusAsc --1
    BEGIN
        SELECT Primary_Event_Num = e.Event_Num
          ...
          LEFT OUTER JOIN Customer_Order_Line_Items l ON l.Order_Line_Id = ed.Order_Line_Id      --10-Dec-2001;mt
          LEFT OUTER JOIN  Customer_Orders co ON co.Order_Id = l.Order_Id                        --10-Dec-2001;mt
          LEFT OUTER JOIN Shipment_Line_Items sl ON sl.Shipment_Item_Id = ed.Shipment_Item_id             --23Jan02;mt
          JOIN  Shipment sh ON sh.Shipment_Id = sl.Shipment_Id AND sh.Shipment_Number = @Shipment_Number  --23Jan02;mt
