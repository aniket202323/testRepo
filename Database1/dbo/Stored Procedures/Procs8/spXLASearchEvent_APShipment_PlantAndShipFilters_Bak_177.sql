/* NOTE ON spXLASearchEvent_APShipment: Derived from spXLASearchEvent, this stored procedure adds the ability to
   ...
   (6) spXLASearchEvent_APShipment_PlantAndShipFilters 	  	  	 'This stored Procedure
   (7) spXLASearchEvent_APShipment_PlantAndCustomerFilters
   (8) spXLASearchEvent_APShipment_PlantCustomerAndShip
*/
-- USAGE: 
-- spXLASearchEvent_APShipment_PlantAndShipFilters should be called when shipment join condition (see above) is met and
-- Plant_Order_Number AND Shipment_Number filters are specified.  MSi/mt/1-25-2002
--
CREATE PROCEDURE dbo.[spXLASearchEvent_APShipment_PlantAndShipFilters_Bak_177] 
  ...
  If @MyType = @NoStringHasStatusAsc --1
    BEGIN
        SELECT Primary_Event_Num = e.Event_Num
          ...
          LEFT OUTER JOIN Shipment_Line_Items sl ON sl.Shipment_Item_Id = ed.Shipment_Item_Id            --25Jan2002:mt
          JOIN Shipment sh ON sh.Shipment_Id = sl.Shipment_Id AND sh.Shipment_Number = @Shipment_Number  --25Jan2002:mt
