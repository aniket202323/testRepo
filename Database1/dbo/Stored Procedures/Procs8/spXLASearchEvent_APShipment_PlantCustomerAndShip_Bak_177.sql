/* NOTE ON spXLASearchEvent_APShipment: Derived from spXLASearchEvent, this stored procedure adds the ability to
   ...
   NOT to add more branches of SQL Statement into spXLASearchEvent_APShipment but to expand into 8 stored procedures listed 
   below. Future bug fix to core code, if any, should be done in all 8 stored procedures.  
   The 8 stored procedures are:
   (1) spXLASearchEvent_APShipment 	  	  	  	 'Master copy
   (2) spXLASearchEvent_APShipment_ShipFilterOnly
   (3) spXLASearchEvent_APShipment_CustomerFilterOnly 	  	 
   (4) spXLASearchEvent_APShipment_CustomerAndShipFilters
   (5) spXLASearchEvent_APShipment_PlantFilterOnly
   (6) spXLASearchEvent_APShipment_PlantAndShipFilters
   (7) spXLASearchEvent_APShipment_PlantAndCustomer
   (8) spXLASearchEvent_APShipment_PlantCustomerAndShip 	  	 'This stored Procedure; name is under 50-char limit 
*/
-- USAGE: 
-- spXLASearchEvent_APShipment_PlantCustomerAndShip should be called when shipment join condition (see above) is met and
-- Plant_Order_Number, Customer_Code, and Shipment_Number filters are specified.  MSi/mt/1-25-2002
--
CREATE PROCEDURE dbo.[spXLASearchEvent_APShipment_PlantCustomerAndShip_Bak_177] 
  ...
  If @MyType = @NoStringHasStatusAsc --1
    BEGIN
        SELECT Primary_Event_Num = e.Event_Num
          ...
          LEFT OUTER JOIN Customer_Order_Line_Items l ON l.Order_Line_Id = ed.Order_Line_Id                    --25Jan02;mt
          JOIN Customer_Orders co ON co.Order_Id = l.Order_Id AND co.Plant_Order_Number = @Plant_Order_Number  --25Jan02;mt
          JOIN Customer c ON c.Customer_Id = co.Customer_Id AND c.Customer_Code = @Customer_Code               --25Jan02;mt
          LEFT OUTER JOIN Shipment_Line_Items sl ON sl.Shipment_Item_Id = ed.Shipment_Item_Id            --25Jan02;mt
          JOIN Shipment sh ON sh.Shipment_Id = sl.Shipment_Id AND sh.Shipment_Number = @Shipment_Number  --25Jan02;mt
