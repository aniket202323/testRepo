/* NOTE ON spXLASearchEvent_NoProduct: Derived from spXLASearchEvent_APMax with the "Applied Product" code removed. Thus
   ...
   (8) spXLASearchEvent_NoProduct_PlantCustomerAndShip 	  	 'This stored Procedure
*/
-- USAGE: 
-- spXLASearchEvent_NoProduct_PlantCustomerAndShip should be called when no-product join condition is met and 
-- Plant_Order_Number, Customer_Code, and Shipment_Number filters are specified. MSi/mt/1-29-2002
--
CREATE PROCEDURE dbo.spXLASearchEvent_NoProduct_PlantCustomerAndShip 
  ...
  If @MyType = @NoStringHasStatusAsc --1
    BEGIN
        SELECT Primary_Event_Num = e.Event_Num
          ...
          JOIN Customer_Orders co ON co.Order_Id = l.Order_Id AND co.Plant_Order_Number = @Plant_Order_Number --29Jan2002;mt
          JOIN Customer c ON c.Customer_Id = co.Customer_Id AND c.Customer_Code = @Customer_Code              --29Jan2002;mt
          LEFT OUTER JOIN Shipment_Line_Items sl ON sl.Shipment_Item_Id = ed.Shipment_Item_Id           --29Jan2002;mt
          JOIN Shipment sh ON sh.Shipment_Id = sl.Shipment_Id AND sh.Shipment_Number = @Shipment_Number --29Jan2002;mt
