/* NOTE ON spXLASearchEvent_NoProduct: Derived from spXLASearchEvent_APMax with the "Applied Product" code removed. Thus
   ...
   (2) spXLASearchEvent_NoProduct_ShipFilterOnly 	  	 'This stored Procedure
   (3) spXLASearchEvent_NoProduct_CustomerFilterOnly 	  	 
...
*/
-- USAGE: 
-- spXLASearchEvent_NoProduct_ShipFilterOnly should be called when no-product join condition is met and 
-- Shipment_Number filter is specified.  MSi/mt/1-25-2002
--
CREATE PROCEDURE dbo.spXLASearchEvent_NoProduct_ShipFilterOnly 
 	 ...
 	 , @Shipment_Number 	 Varchar(50) 	 -- Filter By Shipment_Number; MSi/mt/1-25-2002
...
CREATE TABLE #Events (Event_Num Varchar(50), Event_Id Int, Pu_Id Int, Start_Time DateTime, End_Time DateTime
...
If @ShowStartTime Is NULL SELECT @ShowStartTime = 0
If @ShowStartTime = 1
  BEGIN
    ...
    UPDATE #Events
          SET Start_Time = 
              (SELECT MAX(e.TimeStamp) 
                 FROM Events e 
                WHERE e.Pu_Id = Pu_Id AND e.TimeStamp < End_Time --AND e.TimeStamp > '1970-01-01' (this takes longer to execute)
              )
