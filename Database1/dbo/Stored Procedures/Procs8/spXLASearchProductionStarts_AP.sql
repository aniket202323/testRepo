CREATE PROCEDURE dbo.spXLASearchProductionStarts_AP
 	   @Start_Time  	  	 DateTime
 	 , @End_Time  	  	 DateTime
...
TOP_OF_FETCH_LOOP:
  ...
  If @TimeSort = 1 --Asc
    BEGIN
        SELECT Production_Unit = pu.Pu_Desc,Start_Time = ap.Start_Time at time zone @DBTz at time zone @InTimeZone, End_Time = ap.End_Time at time zone @DBTz at time zone @InTimeZone, p.Prod_Code, Applied_Prod_Code = p2.Prod_Code
...
RETRIEVE_APPLIED_PRODUCT_FILTER_WITHOUT_PRODUCT_CODE:
  ...
  If @TimeSort = 1 --Asc
    BEGIN
       --Select only rows that fall within specified time range  
        SELECT Production_Unit = pu.Pu_Desc,Start_Time = ap.Start_Time at time zone @DBTz at time zone @InTimeZone, End_Time = ap.End_Time at time zone @DBTz at time zone @InTimeZone
          FROM #Applied_Products ap
          JOIN Prod_Units pu ON pu.Pu_Id = ap.Pu_Id
         WHERE ap.Start_Time BETWEEN @Start_Time AND @End_Time
            OR ap.End_Time BETWEEN @Start_Time AND @End_Time
