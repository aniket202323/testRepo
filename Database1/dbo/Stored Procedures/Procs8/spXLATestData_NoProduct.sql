...
--
CREATE PROCEDURE dbo.spXLATestData_NoProduct
 	   @Var_Id 	 Integer
 	 , @Start_Time 	 DateTime
 	 , @End_Time 	 DateTime
 	 , @Pu_Id 	 Integer
...
If @QueryType = @NoEndTimeAscending
    BEGIN
        SELECT  [Result_On] = t.Result_On at time zone @DBTz at time zone @InTimeZone, t.Canceled, 
                  [Result] = CASE 
                                WHEN @DecimalSep <> '.' and @DataType = 2 THEN REPLACE(t.Result, '.', @DecimalSep)
                                ELSE t.Result
                              END,
                [Entry_On] = t.Entry_On at time zone @DBTz at time zone @InTimeZone, t.Comment_Id, e.Event_Id, e.Event_Num, ps.ProdStatus_Desc as 'Event_Status'
          FROM  Tests t 
